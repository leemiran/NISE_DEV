// **********************************************************
//  1. 제      목: 베타테스트시스템 로긴관리
//  2. 프로그램명 : BetaLoginBean.java
//  3. 개      요: 로그인,패스워드찾기
//  4. 환      경: JDK 1.3
//  5. 버      젼: 1.0
//  6. 작      성: 황창근 2004. 11.  14
//  7. 수      정:
// **********************************************************
...
public class BetaLoginBean { 

    public BetaLoginBean() { }
   ...
   public int login(RequestBox box) throws Exception { 
        ...
        try { 
           ...
           // userid,birth_date,name,email, comp, jikup, cono
            sql1  = " select a.userid, a.birth_date, a.name, a.email, ";
            sql1 += "        a.comp, a.jikup, a.jikun, a.cono, a.jikwi, a.jikmu, a.usergubun, b.gadmin  ";
            sql1 += " from TZ_MEMBER a, TZ_MANAGER b                    ";
            sql1 += " where upper(a.userid) = upper(" + StringManager.makeSQL(v_userid) + " )";
            sql1 += "   and a.pwd    = " + StringManager.makeSQL(v_pwd);
            sql1 += "   and a.userid = b.userid ";
            sql1 += "   and b.gadmin in ('T1','A1') ";
            sql1 += "   and b.isdeleted = 'N' ";
            sql1 += "   and to_char(sysdate,'yyyymmdd') between b.fmon and b.tmon ";
			sql1 += "	order by b.gadmin desc";
            ...
            if ( is_Ok == 0 ) { 
	            sql2  = " select a.userid, a.birth_date, a.name, a.email, ";
	            sql2 += "        a.comp, a.jikup, a.jikun, a.cono, a.jikwi, a.jikmu, a.usergubun, b.gadmin  ";
	            sql2 += " from TZ_MEMBER a, TZ_MANAGER b                    ";
           ...
           if ( is_Ok != 0 ) { 
                box.setSession("betauserid", data.getUserid() );
                ...
                box.setSession("jikun", data.getJikun() );
   ...
   /**
    ...
    public int firstLogin(RequestBox box) throws Exception { 
        ...
        try { 
            ...
            if ( v_ismailing.equals("1") ) { 
                ...
                sql += "  where userid = ? and birth_date = ? and name = ?          ";
    ...
    public int sendFormMail(RequestBox box) throws Exception { 
        ...
        try { 
            ...
            sql += " from TZ_MEMBER                          ";
            sql += " where userid = " + StringManager.makeSQL(v_userid);
            sql += "   and birth_date  = " + StringManager.makeSQL(v_birth_date);
...
//                if ( v_isMailing.equals("3") || v_isMailing.equals("")) v_isMailing = "1";
     ...
     */
     public static String getAuthSelect (String userid, String name, String selected, String event) throws Exception { 
         ...
         result = "  <SELECT name='" + name + "' " + event + "  style='border-style:solid;border-width: 1px 1px 1px 1px;border-color:cccccc;color:333333;font-size:9pt;background-color:none;width : 140px;height:19px;font-size:9pt;' > \n";
