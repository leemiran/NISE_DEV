    ...
    public ArrayList SelectMasterFormList(RequestBox box) throws Exception {               
        ...
        try { 
            ...
            sql = "select subj, subjnm, iscentered, dir, isuse, contenttype, "
                ...
                + "  from tz_subj a, tz_cpinfo b where 1 = 1 and a.producer = b.cpseq"
                + "  ";
...
*/
            ...
            while ( ls2.next() ) { 
                ...
                if ( data.getContenttype().equals("N"))  data.setContenttypenm("Normal");
...
//                      isOk = controlLessonBranch(connMgr, p_subj, mfd.getServer(), mfd.getPort(), mfd.getDir(), v_lesson, (String)vc_isbranch.elementAt(j), v_luserid); // 레슨별 분기정보 생성/삭제
            ...
            // Lesson 삭제시 lesson에 연결된 평가마스터,문제지,문제,액티비티,과제,질문방 데이터 변경?
...
// System.out.println(isOk + " == > " + sql);                
            ...
            if ( isOk > 0) { 
                for ( i = 0;i<=1;i++ ) { 
                    // sql = "update " + va_table[i] + " set lesson=dbo.to_char((dbo.to_number(lesson)-1),'00')"
...
// System.out.println(isOk + " == > " + sql);
                ...
                ls.next();
                if ( ls.getInt("CNTS") > 0) { 
                    sql = "select lesson, dbo.to_char((dbo.to_number(lesson)-1),'00') preLesson"
                        + "  from tz_subjlesson "
                        + " where  subj=" +StringManager.makeSQL(p_subj)
                        + "   and  lesson > " +StringManager.makeSQL(p_lesson)
                        + " order by lesson";
