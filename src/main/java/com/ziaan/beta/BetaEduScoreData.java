...
public class BetaEduScoreData { 
    ...
    private double gradexam     = 0;    // 수료기준 평가점수
    ...
    public void makeScoreList() { 
        if ( wstep      > 0 )   makeSub("STEP", wstep, tstep, avtstep, gradstep);
        if ( whtest     > 0 )   makeSub("HTEST", whtest, htest, avhtest, gradexam);
        if ( wmtest     > 0 )   makeSub("MTEST", wmtest, mtest , avmtest, gradexam);
        if ( wftest     > 0 )   makeSub("FTEST", wftest, ftest , avftest, gradexam );
        if ( wreport    > 0 )   makeSub("REPORT", wreport, report, avreport, 0);
        if ( wact       > 0 )   makeSub("ACT", wact, act , avact, 0   );
