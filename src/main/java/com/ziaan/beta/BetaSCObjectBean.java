    ...
    */      
    public ArrayList SelectObjectList(RequestBox box) throws Exception {               
        ...
        try { 
			...
			if ( v_gadmin.equals("S1")||v_gadmin.equals("T1")||v_gadmin.equals("M1") ) { 
             sql += " cpseq='" +v_cpseq + "' and";
            }
            sql += " upper(a.sdesc) like '%" +s_gubun.toUpperCase() + "%'  order by a.oid desc";

            ls = connMgr.executeQuery(sql);
            System.out.println("v_gadmin == = > " +v_gadmin);
            System.out.println("sql=베타테스트" +sql);
            while ( ls.next() ) { 
                data = new BetaSCObjectData(); 
...
// System.out.println(data.getOid() + ":" +data.getLdate() );                
    ...
    */  
    public BetaSCObjectData SelectObjectData(RequestBox box) throws Exception {               
        DBConnectionManager	connMgr	= null;
        ListSet ls = null,ls2= null;
        ArrayList list1 = null, list2= null;
        String sql  = "", sql2= "";
    ...
    **/
     public String InsertObject(RequestBox box) throws Exception { 
        ...
        String  v_oid      = "";
        String  results    = "";
        String  v_producer = box.getStringDefault("p_producer", box.getSession("comp") );
        
        if ( !box.getSession("gadmin").equals("A1") || !box.getSession("gadmin").equals("A2") ) { 
            v_producer = v_cpseq;
        }
        ...
        try { 
            ...
            sql = "select ltrim(to_char( to_number(nvl(max(oid),'1000000000')) +1, '0000000000'  )) oid from tz_object where otype='OBC'";
            ...
            if ( results.equals("OK") ) { 
                ...
                + " ,starting,server,subj,parameterstring,datafromlms "
                + " ,identifier,prerequisites,masteryscore,maxtimeallowed "
                + " ,timelimitaction,sequence,thelevel,luserid,ldate,producer,inuserid,cpseq)"
                + " values "
                ...
                + "," + StringManager.makeSQL(v_producer)
                + "," + StringManager.makeSQL(v_luserid)
                + "," + StringManager.makeSQL(v_cpseq)
                + ")";
...
System.out.println("HEYYYYY" +v_oid);
                sql = "update tz_object set "
                    ...
                    + "     ,subj               = " + StringManager.makeSQL(box.getString("s_subj")) 
                	...
                	if ( isOk == 1) { 
                		sql = "select nvl(max(seq) +1,1) seq from TZ_BETATESTHISTORY";
                		ls = connMgr.executeQuery(sql);
                		if ( ls.next() ) { 
                			v_seq = ls.getInt(1);
                		}
    ...
    */
    public static String makeObcSubjSelect(RequestBox box, String p_selsubj, String p_onchange) throws Exception{ 
        ...
        try { 
            ...
            results += "<option value='NO-SUBJ'";
            if ( p_selsubj.equals("NO-SUBJ")) results +=" selected";
            results +=" >== =과목미연결 == =</option > ";
        }
        catch ( Exception ex ) { 
            throw new Exception("sql = " + sql + "\r\n" + ex.getMessage() );
        }
