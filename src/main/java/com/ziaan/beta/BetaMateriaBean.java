...
public class BetaMateriaBean { 
    private ConfigSet config;
    private int row;
    private String v_type = "CB";	
    public BetaMateriaBean() { 
        try { 
  ...
  /**
   ...
   public DataBox selectPds(RequestBox box) throws Exception { 
        ...
        try { 
            ...
            connMgr.executeUpdate("update tz_board set cnt = cnt + 1 where seq = " + v_seq );        
        }
        catch ( Exception ex ) { 
            ErrorManager.getErrorStackTrace(ex, box, sql);
            throw new Exception("sql = " + sql + "\r\n" + ex.getMessage() );
