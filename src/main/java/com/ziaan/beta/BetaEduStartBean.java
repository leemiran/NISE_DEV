...
public class BetaEduStartBean { 
	...
	* Process
	1. 정상 학습자 체크 : 정상 학습자 아니면 진도데이터 미생성
	2. 기 체크여부 체크 : 기체크되었으면 tz_progress update
	3. 진도제한 체크
	    3.1. 진도제한차시수 체크 
	    3.2. 고용보험과목이면 진도제한 차시수와 비교, 큰 넘으로 진도제한 설정
	4. 고용보험과목이면 이전차시 학습/평가미실시건 존재여부 체크
	5. 진도데이터 생성   
	*/      
	public String EduCheck(RequestBox box) throws Exception {               
		...
		boolean isMake = true;
		String 	sql  = "";
		String	v_mftype, v_isgoyong, v_contenttype= "", results= "";
		String	s_subj		= box.getSession("s_subj");
		String	s_year		= box.getSession("s_year");
		String	s_subjseq	= box.getSession("s_subjseq");
		String	s_userid	= box.getSession("userid");
		...
		if ( p_oid.equals(""))	p_oid = "1";		// oid 없으면 디폴트 OID값 셋팅("1")
		String	p_gubun		= box.getString("p_gubun");
		if ( p_gubun.equals(""))	p_gubun = "END";		// 구분값 없으면 진도체크
		
		String	v_session_time= "", v_total_time= "", v_first_edu= "", v_first_end= "", v_ldate= "", v_sysdate= "";
System.out.println("Start EduCheck: s_eduauth=" +s_eduauth + ".oid=" +p_oid);				
		try { 
			connMgr = new DBConnectionManager();
			// 1.정상 학습자여부 체크
System.out.println("s_eduauthkkkkkkkkkkkkkkkkk == = >>  > " +s_eduauth);
			if ( !s_eduauth.equals("Y"))	return "OK"; 
			// if ( !EduEtc1Bean.isCurrentStudent(s_subj,s_year,s_subjseq,s_userid).equals("Y")) return "OK";
						
			// 2. 기 진도체크여부 
			sql = "select total_time, ldate, to_char(sysdate,'YYYYMMDDHH24MISS') nowtime  "
				 + "  from tz_progress  "
				 + " where subj=" + StringManager.makeSQL(s_subj)
				 + "   and year=" + StringManager.makeSQL(s_year)
				 + "   and subjseq=" + StringManager.makeSQL(s_subjseq)
			...
			if ( !p_oid.equals("1"))	sql += " and oid=" + StringManager.makeSQL(p_oid);
			ls = connMgr.executeQuery(sql);
			...
			// 3. 진도제한 체크
			sql = "select mftype,contenttype  from tz_subj  "
				 + " where subj=" + StringManager.makeSQL(s_subj);
			if ( ls != null ) { try { ls.close(); } catch ( Exception e ) { } }
			ls = connMgr.executeQuery(sql);
			ls.next();
			...
			// edit by leech 
			// 베타테스트는 진도제한없음.
			sql = "select nvl(edulimit,0) limit, isgoyong " + "  from tz_subj "
				 + " where subj=" + StringManager.makeSQL(s_subj);
			// 	 + "   and year=" + StringManager.makeSQL(s_year)
			// 	 + "   and subjseq=" + StringManager.makeSQL(s_subjseq);
			if ( ls != null ) { try { ls.close(); } catch ( Exception e ) { } }
			...
			// 기수상의 진도제한이 7보다 작으면 고용보험기준으로 설정(7차시)
			// if ( v_isgoyong.equals("Y") && v_edulimit < 7)		v_edulimit = 7;		
			// v_edulimit = 0;
			
			if ( v_edulimit != 0) { 
				
				sql = "select count(distinct lesson) CNTS from tz_progress"	
					 + " where subj=" + StringManager.makeSQL(s_subj)
					 + "   and year=" + StringManager.makeSQL(s_year)
					 + "   and subjseq=" + StringManager.makeSQL(s_subjseq)
					 + "   and userid=" + StringManager.makeSQL(s_userid)
				...
				System.out.println("진도 체크 값 쿼리 sql : " + sql);
			...
			sql = "select ldate, first_edu, first_end, to_char(sysdate,'YYYYMMDDHH24MISS') nowtime  from tz_progress  "
				 + " where subj=" + StringManager.makeSQL(s_subj)
				 + "   and year=" + StringManager.makeSQL(s_year)
				 + "   and subjseq=" + StringManager.makeSQL(s_subjseq)
				 + "   and userid=" + StringManager.makeSQL(s_userid)
				 + "   and lesson=" + StringManager.makeSQL(p_lesson)
				 + "   and rownum=1";
            System.out.println("진도체크sql ==  ==  ==  ==  ==  ==  ==  ==  ==  == < > < > < > < > " +sql);
			if ( !p_oid.equals("1"))	sql += " and oid=" + StringManager.makeSQL(p_oid);
			...
			if ( ls.next() ) { 		// update (first_end가 없는 경우이므로 total_time=session_time)
				...
				if ( p_gubun.equals("END") ) { 
					v_session_time  = EduEtc1Bean.get_duringtime(v_ldate,v_sysdate);
					sql +="	,session_time=" +StringManager.makeSQL(v_session_time)
						 + "	,total_time  =" +StringManager.makeSQL(v_session_time)
				...
				if ( isOk == 1)	results = "OK";
				else			results = "진도체크(" +p_gubun + ") 문제가 발생하였습니다. 운영자에게 문의바랍니다.";
				// 점수 조정
				// com.ziaan.complete.FinishBean fb = new com.ziaan.complete.FinishBean(); 
				// isOk = fb.ScoreCompute(box);

				isOk = CalcUtil.getInstance().calc_score(connMgr,CalcUtil.STEP,s_subj,s_year,s_subjseq,s_userid);
			
				if ( isOk == 1)	results = "OK";
				else			results = "진도데이터 생성후 점수조정에 문제가 발생하였습니다.운영자에게 문의바랍니다.";				
			} else { 				// insert (진도시작체크)
				sql = "insert into tz_progress "
					 + "(subj, year, subjseq, userid, lesson , oid, session_time, total_time"
					 + " ,first_edu, first_end, lesson_count, ldate) VALUES "
					 + "(?, ?, ?, ?, ?, ?, '00:00:00.00', '00:00:00.00' "
					 + " ,to_char(sysdate,'YYYYMMDDHH24MISS'),";
				// SET  first_end :종료체크 아니면 null, SET 학습카운트: 종료이면 1, 아니면 0 
				if ( p_gubun.equals("END"))	sql +=" to_char(sysdate,'YYYYMMDDHH24MISS'), 1";
				else						sql +=" null, 0";
				...
				isOk = pstmt.executeUpdate();
				if ( isOk == 1)	results = "OK";
				else			results = "진도시작체크(" +p_gubun + ") 문제가 발생하였습니다. 운영자에게 문의바랍니다.";
		...
		try { 
			...
			while ( ls.next() ) { 
				...
				x.setStarting	 ( ls.getString("starting") );
		       	...
		       	// 평가
		       	sql = "select ptype from tz_exammaster where subj=" +SQLString.Format(p_subj)
		       		 + "   and year='TEST' and subjseq='TEST' and lesson=" +SQLString.Format(x.getLesson() )
		       		 + " order by ptype";
...
System.out.println("sql ==  ==  ==  ==  ==  ==  ==  == =\n" +sql);
    ...
    */	
	public static int get_repCnt(	DBConnectionManager connMgr,
									String	p_subj,
									String	p_year,
									String	p_subjseq,
									String	p_lesson,
	...
	public BetaEduScoreData SelectEduScore(RequestBox box) throws Exception {               
		...
		try { 
			...
			sql = "select a.score score,tstep,htest,mtest,ftest,report,act,etc1,etc2 "            
				 + "		  ,avtstep,avhtest,avmtest,avftest,avreport,avact,avetc1,avetc2"          
				 + " 	  ,gradscore,gradstep,gradexam,wstep,wmtest"
				 + "       ,wftest,whtest,wreport,wact,wetc1,wetc2,isgraduated"
				 + "       ,b.edustart edustart,b.eduend eduend"
				 ...
				 + "   and a.subj=b.subj and a.year=b.year and a.subjseq=b.subjseq";
...
System.out.println("베타테스트 ==  ==  ==  == =" +sql);
			while ( ls.next() ) { 		// ##1	
				...
				if ( !ls.getString("module").equals(v_module)) { 
					...
					d.setAp ( (j-1) + "||" +v_module + "||" +ls.getString("lesson") + "||MODULE1234||MO||SC||MODULE||0||"
							 + ls.getString("MTYPES") + "||N||javascript:whenObj(" +(j-1) + ")||"
							 + ls.getString("modulenm") + "||N");
				...
				if ( !ls.getString("lesson").equals(v_lesson)) { 	
					v_lesson = ls.getString("lesson");
						...
						d = new BetaOBCTreeData();
						d.setAp ( (j-1) + "||" +v_module + "||" +v_lesson + "||LESSON1234||LE||SC||LESSON||0||"
								 + ls.getString("TYPES") + "||N||javascript:whenObj(" +(j-1) + ")||" +ls.getString("lessonnm") + "||N");
				...
				if ( f_go) { 
						...
						sql = "select a.type TYPE, a.oid OID, a.sdesc SDESC, a.ordering ORDERING,"
							 + "		  b.starting STARTING,                    "
						...
						while ( ls1.next() ) { 
							...
							ls2 = connMgr.executeQuery(sql);
							ls2.next();
							if ( ls2.getInt("CNTS") > 0)	f_exist = "Y";
							else						f_exist = "N";
							
							d = new BetaOBCTreeData();
							d.setAp ( (j-1) + "||" +v_module + "||" +v_lesson + "||" +v_oid + "||SC||SC||" +ls1.getString("FILETYPE") + "||" +ls1.getInt("NPAGE") + "||"
									 + v_types + "||" +f_exist + "||javascript:whenObj(" +(j-1) + ")||" +ls1.getString("SDESC") + "||N");
					...
					// 평가 Object 
					// sql = " select count(*) CNTS  from tz_exammaster "
					// 	 + "  where subj=" +StringManager.makeSQL(s_subj)
					// 	// + "    and year='TEST' "
					// 	// + "    and subjseq='TEST' "
					// 	 + "    and lesson=" +StringManager.makeSQL(v_lesson);
					...
					// 		 + " 		 F_EXPURL,F_MULTIEX,Lbirth_date,LDATE,F_USEHTML,F_OPEN,          "
          ...
          // 
		      ...
		      // 
					// 		sql = " select count(*) CNTS from tz_examresult "
					// 			 + "  where subj=" +StringManager.makeSQL(s_subj)
					// 			 + "    and userid=" +StringManager.makeSQL(s_userid)
					// 			 + "    and lesson=" +StringManager.makeSQL(v_lesson)
					// 		   	 + "    and ptype=" +StringManager.makeSQL( ls2.getString("PTYPE") );
					// 		
					// 		if ( ls3 != null ) { try { ls3.close(); } catch ( Exception e ) { } }
					// 		ls3 = connMgr.executeQuery(sql);
					// 		ls3.next();
					// 		if ( ls3.getInt("CNTS") > 0)	f_exist = "Y";
					// 		else						f_exist = "N";
					// 		
					// 		if		( ls2.getString("PTYPE").equals("M"))	v_ptypenm = "Middle";
					// 		else if ( ls2.getString("PTYPE").equals("Q"))	v_ptypenm = "QUIZ";
					// 		else											v_ptypenm = "Final";
					// 		
					// 		d = new BetaOBCTreeData();
					// 		d.setAp ( (j-1) + "||" +v_module + "||" +v_lesson + "||" +v_tmp + "||SC||" +v_extype + "||TEST||0||"
					// 				 + "0001||" +f_exist + "||javascript:whenObj(" +(j-1) + ")||" +v_ptypenm + " Test||N");
        ...
        try { 
            ...
            sql = "select distinct a.oid, a.scotitle, a.scolocate, a.sdesc , a.starting, a.highoid, "
			    + " a.parameterstring, a.datafromlms, a.prerequisites, a.masteryscore,  "
				 ...
				 + " (SELECT lessonstatus FROM tz_progress WHERE subj = '" + p_subj + "'  AND YEAR = '" + p_year + "' "
				 + "  AND subjseq = '" + p_subjseq + "'  AND userid = '" + p_userid + "'  AND OID = b.OID  AND lesson = b.lesson) AS jindostatus "
			    + "from tz_object a , tz_subjobj b   "
			    + " where a.oid = b.oid and  b.subj = '" + p_subj + "' "
