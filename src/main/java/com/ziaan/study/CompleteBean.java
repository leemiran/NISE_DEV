     ...
     public ArrayList selectAdminList(RequestBox box) throws Exception { 
        ...
        try { 
            if ( ss_action.equals("go") ) {  
                ...
                sql1 = "select A.subj,A.subjnm,A.isonoff, A.subjseq,";
                sql1 += "(select count(seq) from TZ_BOARD where tabseq=B.tabseq) cnt,B.tabseq ";
                sql1 += "from vz_scsubjseq A,TZ_BDS B where A.subj=B.subj and A.scsubjseq = B.subjseq and a.year = b.year and B.type='CJ' ";
                ...
                if ( v_orderColumn.equals("") ) { 
                    sql1 += " order by a.subjnm, a.subjseq ";
                } else { 
                    sql1 += " order by " + v_orderColumn + v_orderType + " a.subjseq ";
                }
            
                 System.out.println("sql1 ==  ==  ==  ==  ==  == > 김민수 \n\n " +sql1+ "\n\n");
