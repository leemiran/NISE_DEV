        ...
        try { 
            if ( ss_action.equals("go") ) {  
                ...
                list2 = new ArrayList();                   
                 ...
                 sql1 = "select A.course,A.cyear,A.courseseq,A.coursenm,A.subj,A.isclosed,A.subjnm,A.year,A.subjseq,a.subjseqgr,A.isonoff,A.edustart,A.eduend,";
                    ...
                    while ( ls1.next() ) { 
                     ...
                     for ( int i = 0;i < list1.size(); i++ ) { 
                        ...
                        if ( !v_course.equals("000000") && !(v_Bcourse.equals(v_course) && v_Bcourseseq.equals(v_courseseq))) { 
                            ...
                            sql2 += "where A.subj=B.subj and a.year=b.year and a.subjseq=b.subjseq and B.type='SD' and A.course = '" + v_course + "' and A.courseseq = '" +v_courseseq + "' ";
                  	 		if ( !ss_grcode.equals("ALL") ) { 
...
*/        
        ...
        try { 
            ...
            // ----------------------   게시판 table 에 입력  --------------------------
            sql1 =  " insert into TZ_BOARD(tabseq, seq, userid, name, indate, title, content, cnt, refseq, levels, position, luserid, ldate, gadmin, isimport,subj,year,subjseq)  ";
...
//            sql1 += " values (?, ?, ?, ?, to_char(sysdate,  'YYYYMMDDHH24MISS'), ?, empty_clob(), ?, ?, ?, ?, ?, to_char(sysdate, 'YYYYMMDDHH24MISS'), ?, ?)    ";
            ...
            pstmt1.setString(14, v_subj);
            pstmt1.setString(15, v_year);
            pstmt1.setString(16, v_subjseq);
...
*/        
        ...
        try { 
            ...
            sql1 = "update TZ_BOARD set title = ?, content=empty_clob(), isimport = ? "; //, userid = ?, name = ?, luserid = ?, ldate = to_char(sysdate, 'YYYYMMDDHH24MISS')
