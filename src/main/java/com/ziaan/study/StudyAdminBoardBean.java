...
public class StudyAdminBoardBean { 
...
//        int     l           = 0;
        ...
        try { 
            if ( ss_action.equals("go") ) {  
                ...
                // sql1 = "select A.course,A.cyear,A.courseseq,A.coursenm,A.subj,A.isclosed,A.subjnm,A.year,A.subjseq,A.isonoff,";
...
//                sql1 = "select A.subj,A.subjnm,A.isonoff, a.subjseqgr,";
//                sql1 += "(select count(seq) from TZ_BOARD where tabseq=B.tabseq) cnt,B.tabseq ";
//                sql1 += "from vz_scsubjseq A,TZ_BDS B 											\n"
//                	 +  "where A.subj=B.subj													\n"
//                	 +  "and a.year = b.year                                                    \n"
//                	 +  "and a.subjseq = b.subjseq                                              \n"
//                	 +  "and B.type='SB' 													 	\n";
                
                sql1  = "\n select course, cyear, courseseq, coursenm, a.subj, a.subjnm, a.year, a.subjseq, a.isonoff, a.edustart, a.eduend, a.subjseqgr ";
    			...
    			if ( !ss_subjcourse.equals("ALL") ) { 
    			 ...
    			 if ( !ss_subjseq.equals("ALL") ) { 
    			 	sql1 += "\n and    a.scsubjseq = " + StringManager.makeSQL(ss_subjseq);
    			 }
    			...
    			if ( v_orderColumn.equals("") ) { 
				    ...
				    sql1 += "\n order  by course, cyear, courseseq, " + v_orderColumn + v_orderType;
				}
                    ...
                    while ( ls1.next() ) { 
                     ...
                     for ( int i = 0;i < list1.size(); i++ ) { 
                        ...
                        if ( !v_course.equals("000000") && !(v_Bcourse.equals(v_course) && v_Bcourseseq.equals(v_courseseq))) { 
                            ...
                            sql2 += "where A.subj=B.subj and    a.year = b.year and    a.subjseq = b.subjseq and B.type='SB' and A.course = '" + v_course + "' and A.courseseq = '" +v_courseseq + "' ";
                  	 		if ( !ss_grcode.equals("ALL") ) { 
...
//  
            ...
            // ----------------------   게시판 table 에 입력  --------------------------
            sql1 =  " insert into TZ_BOARD(tabseq, seq, userid, name, indate, title, content, cnt, refseq, levels, position, luserid, ldate, gadmin, isimport)  ";
            sql1 += " values (?, ?, ?, ?, dbo.to_date(getdate(),'YYYYMMDDHH24MISS'), ?, ?, ?, ?, ?, ?, ?, dbo.to_date(getdate(),'YYYYMMDDHH24MISS'), ?, ?)    ";
...
*/        
        ...
        try { 
            ...
            sql1 = "update TZ_BOARD set title = ?, content=?, isimport = ? "; //, userid = ?, name = ?, luserid = ?, ldate = dbo.to_date(getdate(),'YYYYMMDDHH24MISS')
    ...
    */
     public int insertUpFile(DBConnectionManager connMgr, int p_tabseq, int p_seq, RequestBox   box) throws Exception { 
        ...
        try { 
            ...
            sql2 += " values (?, ?, ?, ?, ?, ?, dbo.to_date(getdate(),'YYYYMMDDHH24MISS'))";
    ...
    @return ArrayList   
    */
     public ArrayList selectAdminCommentList(RequestBox box) throws Exception { 
        DBConnectionManager	connMgr	= null;
        ListSet ls1         = null;       
         ListSet ls2         = null;
        ArrayList list1     = null;
...
//        int     l           = 0;
        ...
        try { 
            if ( ss_action.equals("go") ) {  
                ...
                sql1  = "\n select get_grcodenm(a.grcode) grcodenm, course, cyear, courseseq, coursenm, a.subj, a.subjnm, a.year, a.subjseq, a.isonoff, a.subjseqgr ";
	            ...
	            if ( v_orderColumn.equals("") ) { 
	            	...
	            	sql1 += " order  by grcode, course, cyear, courseseq, " + v_orderColumn + v_orderType;
	            }
      ...
      * @throws Exception
      */
      public ArrayList selectCommentListForAdmin(RequestBox box) throws Exception { 
          DBConnectionManager connMgr = null;
          ListSet             ls      = null;
          ArrayList           list    = null;
          String              sql     = "";
          ...
          try { 
              ...
              sql = "select seq																\n"
                ...
                + "  and subjseq = " + StringManager.makeSQL(v_subjseq) + "                	\n";
              	...
              	if ( !v_searchtext.equals("") ) {                //    검색어가 있으면
                    if ( v_search.equals("name") ) {              //    이름으로 검색할때
                        sql += " and get_name(userid) like " + StringManager.makeSQL("%" + v_searchtext + "%");
                    }
                    else if ( v_search.equals("comments") ) {        //    내용으로 검색할때
                        sql += " and comments like " + StringManager.makeSQL("%" + v_searchtext + "%");
                    }
                    else if ( v_search.equals("userid") ) {        //    아이디로 검색할때
                    	sql += " and userid like " + StringManager.makeSQL("%" + v_searchtext + "%");
                    }
                    else if ( v_search.equals("ldate") ) {        //    등록일로 검색할때
                    	sql += " and ldate like " + StringManager.makeSQL("%" + v_searchtext + "%");
                    }
                }
       ...
       * @throws Exception
       */
       public ArrayList selectCommentListForAdmin2(RequestBox box) throws Exception { 
           DBConnectionManager connMgr = null;
           ListSet             ls      = null;
           ArrayList           list    = null;
           String              sql     = "";
       ...
       * @throws Exception
       */
      public int saveCommentListForAdmin(RequestBox box) throws Exception { 
    	  DBConnectionManager	connMgr	= null;
    	  ResultSet rs1 = null;
    	  Statement stmt1 = null;
    	  PreparedStatement pstmt1  = null;
    	  ...
    	  try { 
    		  ...
    		  for ( int	i =	0; i < chkcnt;	i++ ) { 
    			  v_data    = vec_chk.elementAt(i).toString(); 
    			  String[] v_dataArray = v_data.split(",");
    			  v_seq = Integer.parseInt(v_dataArray[0].toString());
    			  v_subj = v_dataArray[1].toString();
    			  v_year = v_dataArray[2].toString();
    			  v_subjseq = v_dataArray[3].toString();
    			  v_userid = v_dataArray[4].toString();
    		  
	    		  sql1 = "update tz_stold_comments		\n" 
	    			  + "    set hidden_yn = 'Y' 			\n"
	    			  + "  where seq = ? 				\n" 
	    			  + "    and subj = ? 				\n"
	    			  + "    and year = ? 				\n"
...
//   			if ( clob != null ) { try { clob.close(); } catch ( Exception e ) { } }
       ...
       * @throws Exception
       */
      public int delCommentListForAdmin(RequestBox box) throws Exception { 
    	  DBConnectionManager	connMgr	= null;
    	  ResultSet rs1 = null;
    	  Statement stmt1 = null;
    	  PreparedStatement pstmt1  = null;
