...
public class CpMateriaBean { 
  private ConfigSet config;
  private int row;
  private String v_type = "AD";
  public CpMateriaBean() { 
    try { 
  ...
  public ArrayList selectPdsList(RequestBox box) throws Exception { 
    ...
    try { 
      ...
      pstmt = connMgr.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE,
  ...
  public DataBox selectPds(RequestBox box) throws Exception { 
    ...
    try { 
      ...
      connMgr.executeUpdate("update tz_board set cnt = cnt + 1 where seq = " +
                            v_seq);
    }
    catch ( Exception ex ) { 
      ErrorManager.getErrorStackTrace(ex, box, sql);
  ...
  public int updatePds(RequestBox box) throws Exception { 
    ...
    try { 
      ...
      if ( !v_newMotionName.equals("") ) { //      수정할 첨부파일이 있다면..
        ...
        /*      Ftp ftp = new Ftp();
                ftp.upload(v_newMotionName);     //      Ftp 서버로 업로드
...
//        sql1 = "update tz_board set title = ?, userid = ?, name = ?, luserid = ?,  content = empty_clob(), indate = to_char(sysdate, 'YYYYMMDDHH24MISS')";
      ...
      sql2 = "select content from tz_board where tabseq = " + v_tabseq +
          " and seq = " + v_seq;
      //     connMgr.setWeblogicCLOB(sql2, v_content);       //      clob 필드에 스트림을 이용,  data 를 넣는다(Oracle Jdbc driver 인 경우)
...
//      connMgr.setOracleCLOB(sql2, v_content); //      (기타 서버 경우)
  ...
  public int deletePds(RequestBox box) throws Exception { 
    ...
    try { 
      ...
      ls.close();
      // ------------------------------------------------------------------------------------
      sql1 = "delete from tz_board where tabseq = " + v_tabseq +
          " and  seq = ?";
...
//// //// //// //// //// //// //// //// //// //// //// //// //// /// 파일 테이블   //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// //// /
  /**
    ...
    */
  public int insertUpFile(DBConnectionManager connMgr, int p_seq,
                          RequestBox box) throws Exception { 
    ListSet             ls      = null;
    ...
    try { 
      if ( realFileNames != null ) { // 
        ...
        // ------------------------------------------------------------------------------------
        // ----------------------   자료 번호 가져온다 ----------------------------
        sql = "select nvl(max(fileseq), 0) from tz_boardfile where tabseq = " +
            v_tabseq;
        ls = connMgr.executeQuery(sql);
        ls.next();
  ...
  /**
    ...
    */
  public int deleteUpFile(DBConnectionManager connMgr, RequestBox box) throws
      Exception { 
    ...
    try { 
      ...
      ls.close();
      // ------------------------------------------------------------------------------------
      sql3 = "delete from tz_boardfile where tabseq = " + v_tabseq +
          " and seq = ? and savefile = ?";
