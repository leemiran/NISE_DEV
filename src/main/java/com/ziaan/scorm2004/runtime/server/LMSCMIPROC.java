...
public class LMSCMIPROC
{
    ...
    */
   public LMSCMIServletResponse doPost( LMSCMIServletRequest iRequest )
   {
         ...
         switch ( mRequest.mRequestType )
         {
            ...
            case LMSCMIServletRequest.TYPE_INIT :
               ...
               */
               ...
               if ( _Debug )
               {
                   System.out.println( "## Tree Object : LMSCMIPROC  읽기 성공" );
               }
               ...
               */
               ...
               if ( _Debug )
               {
                   System.out.println( "## DM Object : LMSCMIPROC 읽기 성공" );
               }
   ...
   private void handleData(SCODataManager iSCOData)
   {
      ...
      try
      {
         ...
         // TODO Navigation 관련 Terminate()시 문제
         scoData.terminate();
         ...
         if ( mRequest.mIsFinished )
         {
            ...
            if ( theTree != null )
            {
               ...
               if ( _Debug )
               {
                   System.out.println( "## Tree Object : LMSCMIPROC  쓰기 성공" );
               }
            }
         }
         ...
         if ( _Debug )
         {
             System.out.println( "## DM Object : LMSCMIPROC 쓰기 성공" );
         }
   ...
   private int setProgressData( DBConnectionManager connMgr, SCODataManager scoData ) throws Exception 
   {
       ...
       PreparedStatement stmtSelectItem = null;
       PreparedStatement stmtUpdateItem = null;
       PreparedStatement stmtUpdateCurriculumUser = null;
       ...
       try
       {
           ...
           if (rsItem.next()) 
           {
               ...
               if ( isFirstEnd ) {
                   // 진행한 차시 수 등록
                   String sqlUpdateCurriculumUser =
                           "\n  update curriculum_user set chasi=                " +
                           "\n  (select count(*) from tz_progress                " +
                           "\n  where subj = ? and year = ? and subjseq = ?      " +
                           "\n  and userid = ? and first_end is not null)        " +
                           "\n  where poi_cd=? and poi_year=? and poi_round=?    " +
                           "\n  and emp_id=?                                     ";

                   stmtUpdateCurriculumUser = connMgr.prepareStatement( sqlUpdateCurriculumUser );
                   stmtUpdateCurriculumUser.setString(1, mSubj);
                   stmtUpdateCurriculumUser.setString(2, mYear);
                   stmtUpdateCurriculumUser.setString(3, mSubjseq);
                   stmtUpdateCurriculumUser.setString(4, mUserID);
                   stmtUpdateCurriculumUser.setString(5, mSubj);
                   stmtUpdateCurriculumUser.setString(6, mYear);
                   stmtUpdateCurriculumUser.setString(7, mSubjseq);
                   stmtUpdateCurriculumUser.setString(8, mUserID);

                   stmtUpdateCurriculumUser.executeUpdate();
                   stmtUpdateCurriculumUser.close();
               }
               */               
           }
       }
       ...
       finally 
       {
           ...
           if(stmtSelectItem != null) { try { stmtSelectItem.close(); } catch (Exception e1 ) {} }
           if(stmtUpdateItem != null) { try { stmtUpdateItem.close(); } catch (Exception e3 ) {} }
           if(stmtUpdateCurriculumUser != null) { try { stmtUpdateCurriculumUser.close(); } catch (Exception e4 ) {} }
       }
