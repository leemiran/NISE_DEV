        ...
        try { 
            ...
            // 수강자수
             v_studentcount = getStudentCount(connMgr, v_grcode, v_gyear, v_grseq, v_subj, v_subjseq, v_sulpapernum, v_company, v_jikwi, v_jikun, v_workplc)
              + getStudentBunrutargetCount(connMgr, v_grcode, v_gyear, v_sulpapernum);
        ...
        try { 
            for ( int i = 0; i < p_answers.size(); i++ ) { 
                ...
                while ( st1.hasMoreElements() && index < p_list.size() ) { 
                    ...
                    if(!v_answer.equals("X")){
                      if ( data.getSultype().equals(SulmunTargetBean.OBJECT_QUESTION)) {   // 단일선택
                          data.IncreasReplyCount(Integer.valueOf(v_answer).intValue() );
                      } else if ( data.getSultype().equals(SulmunTargetBean.MULTI_QUESTION)) {  //다중선택
                          st2 = new StringTokenizer(v_answer,SulmunTargetBean.SPLIT_COLON);
                          while ( st2.hasMoreElements() ) { 
                              v_answer = (String)st2.nextToken();
                              data.IncreasReplyCount(Integer.valueOf(v_answer).intValue() );
                          }
                      } else if ( data.getSultype().equals(SulmunTargetBean.SUBJECT_QUESTION)) {   //주관식
                          subject.add(v_answer);
                      } else if ( data.getSultype().equals(SulmunTargetBean.COMPLEX_QUESTION)) {   //혼합형
                          st2 = new StringTokenizer(v_answer,SulmunTargetBean.SPLIT_COLON);
                          v_answer = (String)st2.nextToken();
                          data.IncreasReplyCount(Integer.valueOf(v_answer).intValue() );
                          String v_sanswer = "";
                      ...
                      } else if ( data.getSultype().equals(SulmunTargetBean.FSCALE_QUESTION)) {  //5점척도
                          data.IncreasReplyCount(Integer.valueOf(v_answer).intValue() );
                      } else if ( data.getSultype().equals(SulmunTargetBean.SSCALE_QUESTION)) {  //7점척도
                          data.IncreasReplyCount(Integer.valueOf(v_answer).intValue() );
                      } 
...
//                sql += " and a.grseq   = " + SQLString.Format(p_grseq);
    ...
    /**
    대상자 설문 설문분석 분류별 target 학생수 구하기 
    @param box          receive from the form object and session
    @return int
    */
    public int  getStudentBunrutargetCount(DBConnectionManager connMgr, String p_grcode, String p_gyear, int p_sulpapernum)  throws Exception { 
        ListSet             ls      = null;
        DataBox             dbox    = null;
        ...
        String 		v_sulgubun 			= "";
        String 		v_seq 				= "";
        String 		v_dept_cd_str	 	= "";
        String 		v_job_cd_str 		= "";
        String 		v_post_str 			= "";
        String 		subj				= "";
        int 		v_count 			= 0;
        String[]	subj_str_all 		= null;
        String		subj_str_detail 		= "";
        String[] 	post_str 			= null;
        String[] 	dept_arr 			= null;
        String[] 	job_arr 			= null;
        String[] 	post_arr 			= null;
        String	 	p_subjseqgr = "";
        String 		p_upperclass = "";
        String 		p_middleclass = "";
        String 		p_lowerclass = "";
        String 		p_subj = "";
        String 		p_subjseq = "";
        ...
        try {
        	...
        	// tz_sulmembertarget 가져오기
        	sql  = " select seq ,searchtext, gubun from tz_sulmembertarget 		\n";
        	sql += " where sulpapernum = " + p_sulpapernum;
            ...
            while ( ls.next() ) { 
            	...
            	v_searchtext 		= dbox.getString("d_searchtext");
            	v_sulgubun 			= dbox.getString("d_gubun");
            	
            	if(!v_searchtext.equals("")){
	            	if(v_sulgubun.equals("1")){	//조직
	            		v_dept_cd_str += v_searchtext.substring(14,20)+"$" ;
	            	}
	            	if(v_sulgubun.equals("2")){// 직급 
	            		post_str = v_searchtext.split("[|]");
	            		for(int i = 0; i < post_str.length ; i++){
	                		v_post_str += post_str[i]+"$"; 
	                	}
	            	}
	            	if(v_sulgubun.equals("3")){//직무
	            		v_job_cd_str += v_searchtext.substring(13,19)+"$" ;
	            	}
	            	if(v_sulgubun.equals("4")){//과정
	            		subj += v_searchtext+"$";
	            	}
            	}
            ...
            ls.close();
            dept_arr 		= v_dept_cd_str.split("[$]")	;
            post_arr 		= v_post_str.split("[$]")	;		
            job_arr 		= v_job_cd_str.split("[$]")	;			
            
        	subj_str_all = subj.split("[$]");
        	

        	// tz_member 의 count세기
            sql1  = " select count(*) cnt from ( 													\n";
            sql1 += " 		select a.userid															\n";
            sql1 += " 		from tz_member a														\n";
            sql1 += " 		where a.dept_cd in ("													   ;
            for(int k =0 ; k < dept_arr.length; k++){
            	 if(dept_arr.length-1 == k){
            		 sql1 +=" '"+dept_arr[k]+"'  " ;
				 }else{
					 sql1 +=" '"+dept_arr[k]+"' ," ;
            ...
            sql1 += " 		select a.userid															\n";
            sql1 += " 		from tz_member a														\n";
            sql1 += " 		where a.post in  (";
        	for(int o =0 ; o < post_arr.length; o++){
           	 if(dept_arr.length-1 == o){
           		 sql1 +=" '"+post_arr[o]+"'  " ;
			 }else{
				 sql1 +=" '"+post_arr[o]+"' ," ;
            ...
            sql1 += " 		select a.userid															\n";
            sql1 += " 		from tz_member a														\n";
            sql1 += " 		where a.job_cd in ( ";
        	for(int s =0 ; s < job_arr.length; s++){
              	 if(job_arr.length-1 == s){
              		 sql1 +=" '"+job_arr[s]+"'  " ;
	   			 }else{
	   				 sql1 +=" '"+job_arr[s]+"' ," ;
            ...
            sql1 += " ) 																				\n";
            for(int j =0 ; j < subj_str_all.length; j++){
        		subj_str_detail = subj_str_all[j];
        		if(!subj_str_detail.equals("")){
        		p_subjseqgr = subj_str_detail.substring(0,4);
                p_upperclass = subj_str_detail.substring(5,8);
                p_middleclass = subj_str_detail.substring(9,12);
                p_lowerclass = subj_str_detail.substring(13,16);
                p_subj = subj_str_detail.substring(17,22);
                p_subjseq = subj_str_detail.substring(23,27);
        		}
            sql1 += " 	union																		\n";
            sql1 += " 		select a.userid															\n";
            sql1 += " 		from tz_member a, tz_subj b, tz_subjseq c, tz_student d					\n";
            sql1 += " 		where a.userid = d.userid												\n";
            sql1 += " 		and b.subj = c.subj														\n";
            sql1 += " 		and c.subj = d.subj														\n";
            sql1 += " 		and c.subjseq = d.subjseq 												\n";
            sql1 += " 		and c.subjseqgr = " + SQLString.Format(p_subjseqgr);
            sql1 += " 		and b.upperclass = " + SQLString.Format(p_upperclass);
            sql1 += " 		and b.middleclass = " + SQLString.Format(p_middleclass);
            sql1 += " 		and b.lowerclass = " + SQLString.Format(p_lowerclass);
            sql1 += " 		and b.subj = " + SQLString.Format(p_subj);
            sql1 += " 		and c.subjseq = " + SQLString.Format(p_subjseq);
            	}
            sql1 += " ) 																			\n";
