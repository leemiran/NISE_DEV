...
public class PermissionBean { 
    ...
    */
    public ArrayList selectListPermission(RequestBox box) throws Exception { 
    ...
    public int insertManager(RequestBox box) throws Exception { 
        ...
        try { 
          ...
          if ( v_cnt > 0 ) {     // 기존 등록되어있으면
		   ...
		   sql +=  " where substr(gadmin,1,2)='" +v_admin + "'";
		   ls = connMgr.executeQuery(sql);
		   if ( ls.next() ) { 
           ...
           sql1 += "  ldate,         ";
           sql1 += "  isview)        ";
           sql1 += "  values(?,'rw',?,?,?,?,?,?,?,?,?,?,?,to_char(sysdate,'YYYYMMDDHH24MISS'), 'Y')               ";
           ...
           pstmt.setString(8, v_isneedoutcomp);
           pstmt.setInt(9, v_applevel);
           pstmt.setString(10, v_admin);
           pstmt.setInt(11, v_gadminseq);
...
/*---------------------------------------------------------------------------------------------------------------------------*/
    ...
    */
    ...
    @return isOk    1:update success,0:update fail
    */
     public int updateGadmin(RequestBox box) throws Exception { 
        DBConnectionManager	connMgr	= null;
        PreparedStatement   pstmt   = null;
		PreparedStatement pstmt1 = null;
		PreparedStatement pstmt2 = null;
        ...
        try { 
            ...
            sql="update tz_gadmin set gadminnm = '슈퍼관리자' where gadmin = 'A1'";
            pstmt = connMgr.prepareStatement(sql);
            isOk = pstmt.executeUpdate();

            sql="update tz_gadmin set gadminnm = '교수' where gadmin = 'P1'";
			pstmt1 = connMgr.prepareStatement(sql);
            isOk = pstmt1.executeUpdate();

            sql="update tz_gadmin set gadminnm = '조교' where gadmin = 'F1'";
