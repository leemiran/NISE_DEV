...
//        String v_week  = FormatDate.getDayOfWeek();
        ...
        try { 
            ...
            sql1 += "   or to_char(sysdate,'YYYYMMDD') = (select substr(lglast,0,8) from TZ_MEMBER where userid =" +StringManager.makeSQL(v_userid) + ")";
			...
			if ( cnt > 0 ) {                        // update
				...
				sql2 += "   and to_char(sysdate,'YYYYMMDD') != (select substr(lglast,0,8) from TZ_MEMBER where userid =?)";
    ...
    */
    public int SelectYearCnt(RequestBox box) throws Exception   { 
        ...
        try { 
			...
			if ( !v_gubun.equals("ALL")) sql += "  and gubun      = " + StringManager.makeSQL(v_gubun);
    ...
    */
    public ArrayList SelectDayTime(RequestBox box) throws Exception   { 
        ...
        try { 
            ...
            sql =  "\nSelect date_year, date_month, date_day, date_time, sum(cnt) cnt ";
            sql += "\n From TZ_HOMECOUNT ";
            sql += "\n Where date_year   = " + StringManager.makeSQL(v_year);
            sql += "\n   and date_month  = " + StringManager.makeSQL(v_month);
            sql += "\n   and date_day    = " + StringManager.makeSQL(v_day);
			if ( !v_gubun.equals("ALL")) sql += "\n and gubun = " + StringManager.makeSQL(v_gubun);
            sql += "\n Group by date_time,date_year, date_month, date_day ";
            sql += "\n Order by date_time asc ";

System.out.println("접속통계(일-시) == > " +sql);
    ...
    */
    public ArrayList SelectMonthWeek(RequestBox box) throws Exception   { 
        ...
        try { 
            ...
            System.out.println("SelectMonthWeek == > " +sql);
