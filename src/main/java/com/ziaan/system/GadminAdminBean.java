...
/*
    public ArrayList searchGadmin(RequestBox box) throws Exception { 
        ...
        try { 
            ...
            sql  = " select gadmin, control, gadminnm, comments, isneedsubj, isneedcomp from TZ_GADMIN   ";

            if ( !v_searchtext.equals("") ) {                            //    검색어가 있으면
                if ( v_search.equals("gadmin") ) {                          //    코드로 검색할때
                    sql += " where comp like   " + StringManager.makeSQL("%" + v_searchtext + "%");
                } else if ( v_search.equals("gadminnm") ) {                 //    명칭으로 검색할때
                    sql += " where deptnm like " + StringManager.makeSQL("%" + v_searchtext + "%");
                }
            }
...
*/
    ...
    /**
    권한코드 셀렉트박스
    @param  name,selected,event,allcheck    셀렉트박스명,선택값,이벤트명,전체유무(0 -전체없음, 1 - 전체있음)
    @return result                          권한코드 + "," + 교육그룹필요여부  + "," + 과목코드필요여부 + "," + 회사코드필요여부 + "," + 부서코드필요여부
    */
    ...
    권한코드 셀렉트박스(강사제외)
    @param  name,selected,event,allcheck    셀렉트박스명,선택값,이벤트명,전체유무(0 -전체없음, 1 - 전체있음)
    @return result                          권한코드 + "," + 교육주관필요여부  + "," + 과목코드필요여부 + "," + 회사코드필요여부 + "," + 부서코드필요여부
    */
