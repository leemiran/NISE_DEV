...
// 			if ( !box.getString("p_upperclass").equals("ALL") ) { 
				sql += " where upperclass = " + SQLString.Format(box.getString("p_upperclass") ); 				
// 			}	
			if ( v_opt.equals("F") || v_opt.equals("P") ) {  // 과목관리자, 강사
...
// 				sql += " where grcode  = " + SQLString.Format(box.getString("p_grcode") );
		...
		} else if(v_callmethod.equals("getCompClass")){		// 본부
			sql  = " select org_cd code, org_nm name from tz_comp   ";
			sql += " WHERE close_dt = '        ' ";
			sql += " AND org_level = '1' ";
			sql += " AND org_cd NOT IN ('100001', '000002') ";
			sql += " ORDER BY BUSINESS_GB, AGENCY_DEPT_GB  ";
			
		} else if(v_callmethod.equals("getJisaClass")){		//지사
			sql  = " select org_cd code, org_nm name from tz_comp   ";
			sql += " WHERE close_dt = '        ' ";
			sql += " AND upper_org_cd = " + SQLString.Format(box.getString("p_compclass"));
			sql += " ORDER BY BUSINESS_GB, AGENCY_DEPT_GB  ";
			
		} else if(v_callmethod.equals("getTeamClass")){		//팀
			sql  = " select org_cd code, org_nm name from tz_comp   ";
			sql += " WHERE close_dt = '        ' ";
			sql += " AND upper_org_cd = " + SQLString.Format(box.getString("p_jisaclass"));
			sql += " ORDER BY BUSINESS_GB, AGENCY_DEPT_GB  ";
			
		} else if(v_callmethod.equals("getBumunClass")){		//부문
			sql   = " SELECT jclass_cd code, jclass_nm name \n";
			sql  += " FROM tk_odu101t \n";
			sql  += " WHERE close_dt='        '  \n";
			
		} else if(v_callmethod.equals("getJikjongClass")){		//직종
			sql   = " SELECT jtype_cd code, jtype_nm name \n";
			sql  += " FROM tk_odu102t \n";
			sql  += " WHERE close_dt = '        ' \n";			
			sql  += " AND jclass_cd = "+ SQLString.Format(box.getString("p_bumunclass"));
			
		} else if(v_callmethod.equals("getJikmugunClass")){		//직무군
			sql   = " SELECT jset_cd code, jset_nm name \n";
			sql  += " FROM tk_odu128t \n";
			sql  += " WHERE close_dt = '        ' \n";			
			sql  += " AND jtype_cd = "+ SQLString.Format(box.getString("p_jikjongclass"));
			
		} else if(v_callmethod.equals("getJikmuClass")){		//직무
			sql   = " SELECT job_cd code, job_nm name \n";
			sql  += " FROM tk_odu103t \n";
			sql  += " WHERE close_dt = '        '  \n";
			sql  += " AND jset_cd = "+SQLString.Format(box.getString("p_jikmugunclass"));
		}
		//System.out.println("*********************************************************************************");
