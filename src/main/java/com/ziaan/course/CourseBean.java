...
import com.ziaan.course.GrseqBean;
...
public class CourseBean { 
    ...
    public ArrayList SelectCourseList(RequestBox box) throws Exception { 
        ...
        try { 
            ...
            if ( !v_coursenm.equals(""))
                sql += "  and a.coursenm like " + SQLString.Format("%" + v_coursenm + "%");
            ...
            while ( ls.next() ) { 
                ...
                data.setSubjectcnt( ls.getInt("subjcnt") );
    ...
    public CourseData SelectCourseData(RequestBox box) throws Exception { 
        ...
        if ( !v_coursenm.equals("") ) { 
            ...
            try { 
                ...
                sql += "      group by course) b ";
                sql += " where a.course = b.course ";
                sql += "   and a.course = " + SQLString.Format(v_course);
    ...
    */
    public ArrayList TargetSubjectList(RequestBox box) throws Exception { 
        ...
        String v_gubun       = box.getString("p_onoffgubun");
        String v_upperclass  = box.getString("s_upperclass");
        String v_middleclass = box.getStringDefault("s_middleclass","ALL");   // 과목분류
        String v_lowerclass  = box.getStringDefault("s_lowerclass","ALL");    // 과목분류
    ...
    */
     public int InsertCourse(RequestBox box) throws Exception { 
        ...
        try { 
            ...
            sql =  "insert into TZ_COURSE(course, coursenm, courseclass, upperclass, middleclass, lowerclass, inuserid, indate, gradscore, gradfailcnt, luserid, ldate) ";
    ...
    */
     public int UpdateCourse(RequestBox box) throws Exception { 
        ...
        try { 
            ...
            // insert TZ_COURSE table
            sql = "update TZ_COURSE set coursenm = ?, courseclass = ?, upperclass = ?, middleclass = ?, lowerclass = ?, gradscore = ?, gradfailcnt = ?, luserid = ?, ldate = to_char(sysdate,'YYYYMMDDHH24MISS') ";
            sql += " where course = ? ";
	...
	*/      
	...
	@return
	*/      
	public static String getCourseSelect(String p_course) throws Exception {               
		DBConnectionManager connMgr = null;
		ListSet ls = null;
		String sql  = "";
		String results="<select name='s_course'><option value='ALL'>ALL</option>";
		String v_course=""; 
		String v_coursenm="";

		try {
			sql = "select distinct course, coursenm "
			...
			while (ls.next()) {
				...
				v_coursenm = ls.getString("coursenm");
				results += "<option value='"+v_course+"'";
				if(p_course.equals(v_course))	results += " selected ";
				results += ">"+v_coursenm+"</option>";					
			}
			results += "</select>";
		}            
		catch (Exception ex) {
	...
	@return
	*/      
	public static String getCyearSelect(String p_cyear) throws Exception {               
		DBConnectionManager connMgr = null;
		ListSet ls = null;
//		ArrayList list1 = null;
		String sql  = "";
...
//			list1 = new ArrayList();
	...
	*/      
	public ArrayList SelectSubjseqList(RequestBox box) throws Exception {               
		...
		try {
			sql = "select a.subj,year,subjseq,a.grcode,b.grcodenm,gyear,grseq			"
				...
				+ "   and a.subj=c.subj ";
			if (!s_upperclass.equals("ALL")) sql+=" and c.upperclass = " + StringManager.makeSQL(s_upperclass);
			if (!s_middleclass.equals("ALL")) sql+=" and c.middleclass = " + StringManager.makeSQL(s_middleclass);
	...
	@return
	*/      
	public static String getYearSelect(String p_year) throws Exception {               
		DBConnectionManager connMgr = null;
		ListSet ls = null;
//		ArrayList list1 = null;
		String sql  = "";
...
//			list1 = new ArrayList();
	...
	@return
	*/      
	public static String getYearSelect(String p_year,boolean pall) throws Exception {               
		DBConnectionManager connMgr = null;
		ListSet ls = null;
//		ArrayList list1 = null;
		String sql  = "";
		String results="<select name='s_year'>";
		if(pall)
			results="<select name='s_year'><option value='ALL'>ALL</option>";
		String	v_year=""; 
...
//		ArrayList list1 = null;
		...
		try {
			sql = "select a.course, a.cyear, a.courseseq, a.coursenm, a.propstart, a.propend, a.edustart, a.eduend,";
			sql+= "	   a.gradscore, a.gradfailcnt,a.grcode, a.gyear, a.grseq, ";
			sql+= "	   b.grcodenm, c.grseqnm ";
	...
	*/      	    
	public int DeleteCourseseq(RequestBox box) throws Exception {
		...
		try {
			...
			isOk = bean.delCourseseq(box.getString("p_course"),box.getString("p_cyear"),box.getString("p_courseseq"),"", connMgr);
			
			System.out.println("isOk====================================="+isOk);
			//sql = "delete from tz_courseseq ";
			//sql+= " where course = "   + SQLString.Format(box.getString("p_course"));
			//sql+= "   and cyear = "    + SQLString.Format(box.getString("p_cyear"));
			//sql+= "   and courseseq = "+ SQLString.Format(box.getString("p_courseseq"));
			//isOk = connMgr.executeUpdate(sql);
