...
//            sql  = "\n select a.*,rownum rowseq from ("
                 sql = "\n select a.faq_type				\n"
                	 ...
                	 + "			, count(*) cnt 			\n"
                	 + "		from tz_cmufaqfile 			\n"
                	 + "		where faq_type = " + StringManager.makeSQL(v_faq_type) + "\n"
                	 + "		group by faq_type, faqno	\n"
                	 + "	) c								\n"
                	 + "where a.register_userid = b.userid 	\n"
                	 + "and a.faq_type = c.faq_type(+) 		\n"
                	 + "and a.faqno    = c.faqno(+)			\n"
                	 + "and a.faq_type = " + StringManager.makeSQL(v_faq_type) + "\n";
                 ;
            ...
            sql += "order by a.root desc, a.position asc	\n";
//                 + "\n ) a";
...
//        int    v_rowseq        = box.getInt("p_rowseq");
        try { 
            ...
            if ( "VIEW".equals(qryFlag)) { 
                sql  ="\n update tz_cmufaq set read_cnt =read_cnt +1"
                ...
                //System.out.println("========selectViewQna============");
            ...
            sql1  = "\n select a.faq_type faq_type,a.faqno faqno,a.title title,a.content content,a.read_cnt read_cnt"
            	 ...
            	 + "\n		from tz_cmufaq x, tz_cmufaq y	"
            	 + "\n		where x.faq_type = y.faq_type	"
            	 + "\n		and x.root = y.root				"
            	 + "\n		and y.lv = (x.lv +1)			" 
            	 + "\n		and x.faqno = a.faqno			"
            	 + "\n   		and y.parent = x.faqno			"
            	 + "\n	) delyn								"
            	 + "\n   from tz_cmufaq a,tz_member b "
                 + "\n        ,(select faq_type faq_type,faqno faqno, count(*) cnt from tz_cmufaqfile where faq_type = '" +v_faq_type + "' group by faq_type,faqno) c"
                 + "\n  where a.register_userid = b.userid "  
            ...
            // 첨부파일읽기
            sql  = "\n select faq_type,faqno,fileno,realfile,savepath,savefile,filesize"
...
//            ls = connMgr.executeQuery(sql);
            ...
            // 댓글읽기
            sql  = "\n select a.faq_type faq_type,a.faqno faqno,a.rplno rplno,a.content content,a.userid userid"
