...
public class DatabaseExecute {
    ...
    */    
    public boolean executeUpdate() throws Exception {
        ...
        try {
            if(connMgr == null) this.connMgr = new DBConnectionManager();                   
            
            Vector tables = box.getVector("p_table");
     
            if(tables.size() > 1) connMgr.setAutoCommit(false);        //      다수 테이블 실행할 때 트랜젝션 처리
            ...
            if(!box.getBoolean("notTransaction")) doCommit = this.isCommit();//System.out.println(getSQLString());
            //System.out.println(getIsCommitString());
...
//------------------------------------------------------------------------------------------------
    ...
    */  
     public boolean executeUpdate(PreparedBox [] pbox, String [] p_sql) throws Exception {    
        ...
        int isOk = 0;       
         ...
         try {
            ...
            if(pbox.length > 1) connMgr.setAutoCommit(false);        //      다수 테이블 실행할 때 트랜젝션 처리
            
            h_sql = new Hashtable();
            h_isCommit = new Hashtable();
            //System.out.println("pbox.length " +pbox.length);
            for(int k = 0; k < pbox.length; k++) {    
                int beforeIsOk = 0;
                isOk = 0;        
                String pstmtSet = "";
                  
                pstmt = connMgr.prepareStatement(p_sql [k]);
                ///System.out.println("sql = " +p_sql [k]);
    ...
    //------------------------------------------------------------------------------------------------
            ...
            if(!box.getBoolean("notTransaction")) doCommit = this.isCommit();
            System.out.println("doCommit" + doCommit);                 
            if(doCommit) connMgr.commit();
            else connMgr.rollback();
        }     
...
//        System.out.println("result" + result);
    ...
    private String getJustColumn(String p_tableName, String p_columnName) throws Exception {
        ...
        try {
            ...
            if( !box.containsKey("p_" + result)) result = "";        
        }
        catch(Exception e) {
//            e.printStackTrace();
...
//            e.printStackTrace();
    ...
    private String setData(PreparedStatement pstmt, int i, String ls_type, String ls_value) throws java.sql.SQLException , Exception{
        ...
        /*
        if (ls_value.length() > 60) {
            pstmt.setCharacterStream(i, new BufferedReader(new StringReader(ls_value)), ls_value.length());
        }
        else {
            pstmt.setString(i,ls_value);
        }
        ...
        */
        /*
        else if (ls_type.equals("DATE")) {
            if (ls_value==null || ls_value.equals(""))
            ls_value="1900/01/01/00/00";
            java.sql.Timestamp ts_value = DateTime.getRandomdate(ls_value);
