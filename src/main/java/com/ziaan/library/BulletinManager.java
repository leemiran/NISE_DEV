...
public class BulletinManager { 
    ...
    */
    public synchronized static RequestBox getState(String name, RequestBox box, PrintWriter out) throws Exception {       
     ...
     /**
    * 로그인한 사용자가  이 게시판에서  읽기/추가/수정/삭제/답변/파일업로드여부/답변기능여부/커뮤니티여부, 본인작성글 수정과 같은 권한여부를 리턴한다.
    @param name  Servlet Name
    @param box  RequestBox class
    ...
    public synchronized static boolean isAuthority(RequestBox box, String authority) throws Exception {     
        ...
        try {        
            ...
            if ( authority != null && gadmin != null ) {                        
                ...
                if ( auth == 0) isAuth = true;         //      같은 권한인 경우
                else if ( s_userid.equals(writer)) isAuth = true;            //      본인인 경우(로그인한 사람 = 게시글 작성자)
                else if ( box.getBoolean("p_isCommunity") && box.getString("p_isleader").equals("Y")) isAuth = true;            //      커뮤니티에서 시삽인 경우
                else isAuth = false;                              
            }
        } catch ( Exception ex ) { 
            ErrorManager.getErrorStackTrace(ex);
            Log.sys.println(ex, "Happen to BulletinManager.isAuthority(\"" + authority + "\")");      
