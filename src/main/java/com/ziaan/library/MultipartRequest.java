...
public class MultipartRequest { 
    ...
    */
    ...
    /** 
    * 업로드되는 file 의 'name', 'realFileName'(클라이언트에서 업로드되는 파일명) 값을 저장하고 있는 hashtable 객체
    */
    public Hashtable realFileParameters = new Hashtable();      //     클라이언트에서 업로드되는 파일명들
    
    /** 
    * 업로드되는 file 의 'name', 'newFileName'(서버에 저장될 새로운 파일명) 값을 저장하고 있는 hashtable 객체
    */
    public Hashtable newFileParameters = new Hashtable();      //      서버에 저장될 새로운 파일명들    
    
    /** 
    * 파일이 서버에 업로드가 제대로 이루어졌는가를 저장하는 hashtable, 
...
// System.out.println(" servletName ==  ==  ==  ==  ==  ==  ==  == = > " + servletName );
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    public MultipartRequest(ServletRequest request, String saveDirectory, String servletName, String userid, int maxPostSize) throws Exception { 
...
// System.out.println(" servletName ==  ==  ==  ==  ==  ==  ==  == = > " + servletName );
    ...
    *request 객체를 받아 box hashtable 에 parameters 와 원파일명, 새로운파일명을 담아 반환한다.
    ...
    @return RequestBox request 객체에서 받은 파라미터 name 과 value, realfilename, newfilename 을 담은 hashtable 객체를 반환함
    */
    public RequestBox getBox(HttpServletRequest request) { 
        ...
        while ( e.hasMoreElements() ) { 
            ...
            Object o = (Object)parameters.get(key);
            Class c = o.getClass();
            if ( o != null ) { 
                if ( c.isArray() ) { 
                	box.put(key, (String [])parameters.get(key));
                } else { 
        ...
        while ( e1.hasMoreElements() ) { 
            ...
            Object o1 = (Object)realFileParameters.get(key);
            if ( o1 != null ) { 
                box.put(key + "_real", (Vector)realFileParameters.get(key));
            }
            
            Object o2 = (Object)newFileParameters.get(key);
            if ( o2 != null ) { 
                box.put(key + "_new", (Vector)newFileParameters.get(key));
            }            
        }        
        return box;
    }
    ...
    */
    ...
    /**
    * 업로드때 넘어오는 파라미터들은 'parameters' 에 name 별로 구분하여 value를 저장하고 
    * 파일은 'files' 에 file명 별로 구분하여 UploadedFile 객체를 저장한다
    @param in MultipartInputStreamHandler 객체를 인자로 받는다
    @param boundary 분리할 부분을 나누어주는 표식자를 받는다
    @return 마지막 부분인지 여부를 반환한다
    @exception Exception 만일 read하거나 parsing 할때 문제가 발생하면 예외처리한다.
    * request
    @see readParameter
    @see readAndSaveFile
    */
        ...
        if ( filename == null || filename.equals("") ) {      // This is a param
            ...
            if ( blChk) { 
                ...
                if ( c.isArray() ) { 
                    ...
                    // 					System.out.println("Exist length=" +existArray.length + ",new=" +newArray.length);
                    ...
                    // 						System.out.println("newArray[" +k + "]-" +newArray[k]);
                    // 					}	
                } else { 
...
/*    protected String getSaveFileName(String paramName, String realFileName) { 
        ...
        if ( !(newFileName == null || newFileName.equals("")) || realFileName.equals("") ) {     // 파일이름이 "" 이 아니고 지정된 새이름이 null 이거나 "" 이 아니면      
            ...
            if ( !fileExtension.equals("") ) { 
                newFileName = newFileName + "." + fileExtension;       // 새로운 파일명을 만든다
            }
        }
        else {      //  새이름이 null 이거나 공백일경우 / 업로드파일의 이름이 공백일경우 (null 일수는 없다) 는 기존의 파일명을 그대로 사용한다
   ...
   * 업로드되는 파일을 읽고 새로운 파일명으로 저장한다.
   ...
   @return retval  설정한 최대 파일사이즈와 비교하여 최대파일사이즈보다 크면 삭제한다. 0 을 반환한다.  정상처리되면 1 을 반환한다.  
   */
