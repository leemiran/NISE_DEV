...
public class EgovSysInfo {
	...
	*/
	public static String getPrductStatus(String port) throws Exception {
		...
		try
		{   
			String cmdStr = EgovProperties.getProperty(Globals.SERVER_CONF_PATH, "SHELL."+Globals.OS_TYPE+".getPrductStatus");
	...
	*/
	public static String getOSName() throws Exception {
		...
		try
		{   
			String cmdStr = EgovProperties.getProperty(Globals.SERVER_CONF_PATH, "SHELL."+Globals.OS_TYPE+".getOSInfo");
            String[] command = {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
            					"NAME"};
            p = Runtime.getRuntime().exec(command);
		    //p.waitFor();
	...
	*/
	public static ArrayList getDiskName() throws Exception {
		...
		try
		{   
			String cmdStr = EgovProperties.getProperty(Globals.SERVER_CONF_PATH, "SHELL."+Globals.OS_TYPE+".getDiskInfo");
            String[] command = {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
            					"NAME"};
            p = Runtime.getRuntime().exec(command);
		    //p.waitFor();
	...
	*/
	public static float getMoryFullCpcty() throws Exception {
		...
		try
		{   
			String cmdStr = EgovProperties.getProperty(Globals.SERVER_CONF_PATH, "SHELL."+Globals.OS_TYPE+".getMoryInfo");
            String[] command = {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
            					"FULL"};
            p = Runtime.getRuntime().exec(command);
		    //p.waitFor();
    ...
    */
    public static ArrayList executeProgram(String propertyKeyword, String[] command) throws Exception {
        ...
        {       
            Process p = null;
            String cmdStr = EgovProperties.getProperty(Globals.SHELL_FILE_PATH, "SHELL."+Globals.OS_TYPE+"."+propertyKeyword);
            command[0]=cmdStr;
            p = Runtime.getRuntime().exec(command);
            p.waitFor();
            //프로세스 에러시 종료
