	...
	public String getEncriptData(HttpServletRequest request, HttpServletResponse response,String retType) {
		...
		try {
			...
			*********************************************************************************************************************************************/
			...
			String sSiteCode				= "EP60";			// NICE평가정보에서 발급한 IPIN 서비스 사이트코드
			String sSitePw					= "!Epasskorea0429!";		// NICE평가정보에서 발급한 IPIN 서비스 사이트패스워드
			...
			└────────────────────────────────────────────────────────────────────
			*/
			String sReturnURL				= "http://iedu.nise.go.kr/cert/ipin/ipinResult.do?retType=" + retType;
			...
			*/
			...
			if (iRtn == 0)
			{
				...
				reqInfo = pClient.getCipherData();		//암호화 된 데이터
			}
		...
		try {
		    ...
		    if (!sResponseData.equals("") && sResponseData != null)
		    {
				...
				if (iRtn == 1)
				{
					...
					name = pClient.getName();			// 이름
					birYMD = pClient.getBirthDate();		// 생년월일 (YYYYMMDD)
					if(pClient.getGenderCode().equals("1"))
					{
						sex = "M";
					}
					else
					...
					fgnGbn = pClient.getNationalInfo();	// 내/외국인 정보 (개발 가이드 참조)
					di = pClient.getDupInfo();			// 중복가입 확인값 (DI - 64 byte 고유값)
					ci1 = "";
					ci2 = "";
					civersion = "";
					reqNum = "";
