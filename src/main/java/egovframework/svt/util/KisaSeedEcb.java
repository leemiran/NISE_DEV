...
public class KisaSeedEcb {
	
	// DEFAULT : JAVA = BIG_ENDIAN
	private static int ENDIAN = Common.BIG_ENDIAN;
	
	private static final String encrypt_key = "2B,7Y,02,17,28,AC,D2,A6,AB,F7,15,4b,09,CN,4F,3c";
	...
	public static String SEED_ECB_Encrypt(String value) {
		
		String linkKey = encrypt_key;
		String hexText = "";
		String plainTextStr = convertStringToHex(value);
		
    	for(int i=0; i < plainTextStr.length(); i=i+2) {
    		hexText = hexText + plainTextStr.substring(i, i+2) + ",";
    	}
    	
		byte[] pbszUserKey = getBytes(linkKey);
    	
    	hexText = hexText.substring(0, hexText.lastIndexOf(","));
    	
    	plainTextStr = hexText;
    	
    	byte[] pbData = getBytes(plainTextStr);
		...
		return getString(pbszCipherText);
	}	
	...
	*/
	...
	public static String SEED_ECB_Decrypt(String value) {
		...
		byte[] pbszCipherText = getBytes(value);
		...
		if (SEED_ECB_Close( info, outbuf, nRetOutLeng[0], nPaddingLeng ) > 0 )
		{
	    	...
	    	String plainTextStr = getString(result);
	    	String haxData = plainTextStr.replace(",", "");
	    	plainTextStr = convertHexToString(haxData);
