...
public class DateTimeValidator extends DMTypeValidator implements Serializable
{
   ...
   private final int SECOND_UPPER_BOUND = 59;

   private final int MIN_UPPER_BOUND = 59;

   private final int HOUR_UPPER_BOUND = 23;

   private final int MONTH_UPPER_BOUND = 11;

   private final int YEAR_LOWER_BOUND = 1970;

   private final int YEAR_UPPER_BOUND = 2038;
    ...
    * 
    * @param Indicates if subseconds should be tested for in this validator.
    */
   ...
   public boolean compare(String iFirst, String iSecond, Vector iDelimiters)
   {
      ...
      DateTimeParser dtp = (ISODateTimeFormat.getInstance()).dateTimeParser();
   ...
   public int validate(String iValue)
   {
      ...
      if ( !done )
      {
         ...
         try
         {
            year = Integer.parseInt(iValue.substring(0, 4), 10);
            ...
            if ( idx != iValue.length() )
            {
               ...
               if ( iValue.charAt(idx) == '-' )
               {
                  ...
                  try
                  {
                     month = 
                     Integer.
                     parseInt(iValue.substring(idx, idx + 2), 10);
         ...
         if ( !done )
         {
            ...
            if ( idx != iValue.length() )
            {
               if ( iValue.charAt(idx) == '-' )
               {
                  ...
                  try
                  {
                     day = Integer.parseInt(iValue.substring(idx, idx + 2), 10);
      ...
      if ( !done )
      {
         ...
         if ( idx != -1 )
         {
            ...
            try
            {
               hour = Integer.parseInt(iValue.substring(idx, idx + 2), 10);
               ...
               if ( !done )
               {
                  ...
                  if ( idx != iValue.length() )
                  {
                     if ( iValue.charAt(idx) == ':' )
                     {
                        ...
                        {
                           seconds =
                           Integer.parseInt(iValue.substring(idx, idx + 2), 10);
                  ...
                  if ( mIncludeSubSecs )
                  {
                     if ( idx != iValue.length() )
                     {
                        if ( iValue.charAt(idx) == '.' )
                        {
                           ...
                           {
                              // Found range now verify they are numbers
                              int subseconds = -1;
                              try
                              {
                                 subseconds =
