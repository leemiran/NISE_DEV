...
SeqReportActivityStatus, Serializable
{
   ...
   /**
    * Indicates where a 'flow' sequencing request failed at.
    */
   private SeqActivity mFlowFailure = null;
    ...
    *
    * @see <code>SeqReportActivityStatus</code>
    */
      ...
      if ( target != null )
      {
         ...
         if ( target.getIsActive() && target.getIsTracked() )
         {
            ...
            if ( !target.hasChildren(false)  &&
                 mSeqTree.getCurrentActivity() == target )
            {
               boolean statusChange = target.setObjMeasure(iObjID, iMeasure);
   ...
   public ADLLaunch navigate(int iRequest)
   {
      ...
      if ( newSession && ( iRequest == SeqNavRequests.NAV_EXITALL ||
   ...
   private void validateRequests()
   {
      ...
      if ( cur != null )
      {
         ...
         if ( cur.getIsActive() )
         {
            ...
            if ( _Debug )
            {
               System.out.println("  ::--> Performing Flow Validation");
            }

            // Perform a tentative 'exit' termination request
            seqReq = doTerminationRequest(ADLSequencer.TER_EXIT, true);

            if ( mValidTermination )
            {
               // If the termination request resulted in a sequencing request,
               // process it.
               if ( !mExitCourse && seqReq != null )
               {
                  delReq = doSequencingRequest(seqReq);

                  if ( mValidSequencing )
                  {
                     seqReqSuccess = doDeliveryRequest(delReq, 
                                                       true,
                                                       tempLaunch);
                  }
               }

               // If the termination request resulted in a valid activity or an
               // end to the seq session -- set 'Continue' to valid
               if ( mExitCourse || seqReqSuccess )
               {
                  if ( _Debug )
                  {
                     System.out.println("  ::--> AUTO Continue");
                  }

                  valid.mContinue = true;

                  SeqActivity tmpParent = cur.getParent();
                  if ( tmpParent != null )
                  {
                     if ( tmpParent.getControlModeFlow() &&
                          !tmpParent.getControlForwardOnly() )
                     {
                        if ( _Debug )
                        {
                           System.out.println("  ::--> AUTO Previous");
                        }

                        valid.mPrevious = true;
                     }
                  }
               }
               else
               {
                  ...
                  test = doIMSNavValidation(SeqNavRequests.NAV_CONTINUE);

                  if ( test )
                  {
                     // Test the 'continue' request
                     mValidSequencing = true;

                     delReq = doSequencingRequest(ADLSequencer.SEQ_CONTINUE);
                     if ( mValidSequencing )
                     {
                        valid.mContinue = doDeliveryRequest(delReq,
                                                            true,
                                                            tempLaunch);
                     }

                     // If the 'continue' request is invalid, check for
                     // a flowing exit
                     if ( !valid.mContinue )
                     {
                        if ( _Debug )
                  ...
                  if ( test )
                  {
                     ...
                     {
                        valid.mPrevious =
                        doDeliveryRequest(delReq, true, tempLaunch);
                     }
                  }
               }
            }
         ...
         else
         {
            if ( _Debug )
            {
               System.out.println("  ::--> Current Activity is not Active");
               System.out.println("  ::--> Validate 'Continue'");
            }
            ...
            if ( test )
            {
               ...
               if ( mValidSequencing )
               {
                  valid.mContinue = doDeliveryRequest(delReq, true, 
                                                      tempLaunch);
               }
            }
      ...
      else
      {
         if ( _Debug )
         {
            System.out.println("  ::--> No current activity -- only choice");
         }
...
//               invokeRollup(cur, null);
    ...
    *         <code>null</code>.
    * @see ADLSequencer TER_[xxx] constants
    * @see ADLSequencer SEQ_[xxx] constants
    */
...
//            }
   ...
   private void invokeRollup(SeqActivity ioTarget, Vector iWriteObjIDs)
   {
      ...
      while ( rollupSet.size() != 0 )
      {
         if ( _Debug )
         {
            ...
            Enumeration enume = rollupSet.keys();
      ...
      if ( ioCluster.getChildren(true) != null )
      {
         ...
         if ( ioCluster.getReorderChildren() )
         {
            ...
            for ( int i = 0; i < children.size(); i++ )
            {
               ...
               {
                  rand = gen.nextInt();
                  num = Math.abs(rand % children.size());
                  ...
                  {
                     set.add(new Integer(num));
                     reorder.add(children.elementAt(num));
                     OK = true;
                  }
               }
            }
            ...
            if ( process )
            {
               ...
               else
               {
                  ...
                  if ( process && con != null )
                  {
                     ...
                     {
                        success = processFlow(FLOW_FORWARD, false, 
                                              walkCon, true);
                     }
                     else
                     {
                        success = processFlow(FLOW_BACKWARD, false,
                                              walkCon, true);   
                     }
                         ...
                         boolean iControl)
   {
      ...
      // Test if we have skipped all of the children in a 'forward-only' clus
      // traversing backward
      if ( !done && parent != null )
      {
      ...
      if ( !done && direction == ADLSequencer.FLOW_FORWARD )
      {
         if ( iFrom.getID().equals(mSeqTree.getLastLeaf()) )
         {
            ...
            // We are at the last leaf of the tree
            done = true;
...
//            }
                               ...
                               Walk ioFrom, boolean iConChoice)
   {
      ...
      // If the process failed, indicate where it failed at
      if ( !success )
      {
         mFlowFailure = ioFrom.at;
      }
   ...
   private boolean evaluateLimitConditions(SeqActivity iTarget)
   {
      ...
      // -+- TODO -+-
      // For Beta 2, we only test max attempt limits
   ...
   private void endAttempt(SeqActivity iTarget, boolean iTentative)
   {
      ...
      if ( iTarget != null )
      {
         ...
         // The activity becomes inactive if this isa 'real' termination
         if ( !iTentative )
         {
   ...
   private Vector getTOC(SeqActivity iStart)
   {
      ...
      while ( !done )
      {
         ...
         if ( include )
         {
            ...
            else
            {
               ...
               if ( walk.getPreventActivation() )
               {
                  if ( cur != null )
                  {
                     if ( walk != cur && walk.getParent() != cur )
                     {
                        if ( _Debug )
                        {
                           System.out.println("  ::--> PREVENTED !!");
                           System.out.println(" " + walk.getParent().getID() +
                                              " != " + cur.getID());
                        }
         ...
         if ( include )
         {
            ...
            if ( cur != null )
            {
               if ( temp.mID.equals(cur.getID()) )
               {
                  ...
                  // We just added the current activity, test the parent for
                  // the Forward Only control mode
                  if ( parent == null || parent.getControlForwardOnly() )
                  {
                     if ( _Debug )
                     {
                        System.out.println("  ::--> Current Activity is in " +
                                           "a forward only cluster");
                     }

                     // Walk backward through the existing TOC and mark
                     // selectable previous siblings un-selectable
                     int idx = toc.size() - 1;
                     while ( idx >= 0 )
                     {
                        ADLTOC prev = (ADLTOC)toc.elementAt(idx);

                        // Confirm this activity is a sibling of the current
                        // activity
                        if ( prev.mDepth == depth )
                        {

                           if ( prev.mIsSelectable )
                           {
                              prev.mIsSelectable = false;

                              if ( _Debug )
                              {
                                 System.out.println("  xxx:: " + prev.mID +
                                                    " ::xxx");
                              }
                           }
                        }
            ...
            else
            {
               ...
               {                 
                  ...
                  if ( tempAct.getPreventActivation() )
                  {
                     if ( cur != null )
                     {
                        if ( tempAct != cur && tempAct.getParent() != cur )
                        {
                           if ( _Debug )
                           {
                              System.out.println("  ::--> PREVENTED !!");
                              System.out.println(" " + tempAct.getParent().getID() +
                                                 " != " + cur.getID());
                           }
      ...
      if ( con != null )
      {
         ...
         success = processFlow(FLOW_FORWARD, false, walkCon, true);
         ...
         {
            int size = list.size();
            forwardAct = ((SeqActivity)list.elementAt(size - 1)).getCount();
         }
         else
         {
            forwardAct = walkCon.at.getCount();
         }

         // Find the previous activity relative to the constrained activity.
         walkCon.at = con;
         success = processFlow(FLOW_BACKWARD, false, walkCon, true);
         ...
         backwardAct = walkCon.at.getCount();
         ...
         for ( int i = 0; i < toc.size(); i++ )
         {
            ...
            if ( temp.mCount < backwardAct || temp.mCount > forwardAct )
            {
               temp.mIsSelectable = false;
      ...
      if ( _Debug )
      {
         System.out.println("  ::--> Completed forth pass");
      }
