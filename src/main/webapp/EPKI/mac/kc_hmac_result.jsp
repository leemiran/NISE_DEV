<%@page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" import="com.epki.api.EpkiApi,
...
<%
    ...
    try
    {
        ...
        // Mac 객체 생성
        Mac mac = Mac.getInstance(strAlgID);

        // 사용자가 입력한 Password를 통하여 MAC Data 생성
        bInputData = strMessage.getBytes();
        bMACData = mac.generate(strPassword, bInputData);

        // MAC Data 검증
        mac.verify(strPassword, bInputData, bMACData);

        // MAC Data Base64 인코딩
        Base64 encoder = new Base64();
        strMACData = encoder.encode(bMACData);

        strVerifyResult = "MAC 검증 성공";
    } catch (EpkiException e) {
        strVerifyResult = "MAC 검증 실패" + e.toString();
    }
%>
                            ...
                            <table class="table table-bordered">
                                <tr>
                                    ...
                                    <td class="table-input-text">
                                        <textarea ondblclick="textareaResize(this)"><%=strVerifyResult%></textarea>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="table-input-name">
                                    ...
                                    <td class="table-input-text">
                                        <textarea ondblclick="textareaResize(this)"><%=strPassword%></textarea>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="table-input-name">
                                    ...
                                    <td class="table-input-text">
                                        <textarea ondblclick="textareaResize(this)"><%=strMACData%></textarea>
                                    </td>
                                </tr>
                            </table>
                        </form>
