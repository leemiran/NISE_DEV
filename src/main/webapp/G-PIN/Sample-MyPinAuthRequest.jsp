...
<%
    /**
     * My-PIN 본인인증을 요청하는 페이지입니다.
     * 오프라인상에서 본인인증이 필요한 경우에 이 페이지를 호출하시면 됩니다.
     * 인증이 완료되면 session에 사용자정보가 설정됩니다.
     * 설정된 사용자 정보를 참조하는 방법은 Sample-MyPinAuthResponse를 참조하시기 바랍니다.
     */
    // 인증완료후 session에 저장된 사용자정보를 참조할 페이지, (이용기관 인증수신페이지와 다릅니다.)
    // TODO 이용기관에서 사용하실 페이지를 지정합니다.
    session.setAttribute("gpinAuthRetPage", "Sample-MyPinAuthResponse.jsp");
    ...
    try {
    	requestHTML = proxy.makeMypinAttributeRequest(	request.getParameter("mypinNumber")
    													, request.getParameter("requestAuthMethod")
    													, name
