...
<%!
public class JLRptTree extends JLTreeCtrl
{
	JLRptTree(javax.servlet.jsp.JspWriter out)
	{
		super(out);
		setClassName("JLRptTree");
	}
	...
	Vector getSegment(String sParent)
	{
		...
		String sSql = "SELECT   id,name, parents,(SELECT   COUNT(*) cnt FROM      schedule_folder2 s WHERE   s.parents = p.id) AS childcount FROM      schedule_folder2 p WHERE   (p.parents = '"+sParent+"')";
	...
	String[] onInsertNode(String sArg[])
	{
		...
		*/
		...
		{		
			...
			String schedule = sArg[1];// "Schdule Test";
			String parents = sArg[7];//"1";
			String sSql = "INSERT INTO schedule_folder2 (name,parents,guid) VALUES ('"+schedule+"',"+parents+",'"+sGuid+"')";
			debugPrint(sSql);
			//if (true) return;
		...
		{		
			...
			String sSql = "select id from schedule_folder2 where guid = '"+sGuid+"'";
			debugPrint(sSql);
			//if (true) return;
	...
	String[] onUpdateNode(String sArg[])
	{
		...
		*/
		...
		{		
			...
			String schedule = sArg[1];// "SchduleTest";
			// String parents = sArg[7];//"1";
			String sSql = "update schedule_folder2 set name = '"+schedule+"' where id = " + sArg[0];
			debugPrint(sSql);
			//if (true) return;
	...
	String[] onDeleteNode(String sArg[])
	{
		...
		*/
		...
		{		
			...
			// String schedule = sArg[1];// "Schedule Test";
			// String parents = sArg[7];//"1";
			String sSql = "delete schedule_folder2  where id = " + sArg[0];
			debugPrint(sSql);
			//if (true) return;
