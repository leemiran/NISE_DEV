<%@ page import="java.io.*,java.util.*,m2soft.rdsystem.server.log.m.*,m2soft.rdsystem.server.logger.*,m2soft.rdsystem.server.core.rddbagent.jdbc.L" %>
<%@ page session="false" %>
...
<%
	response.setHeader ("Refresh", rMilisecond+"; URL='JLogview.jsp?filename="+RdLogManager.getInstance().getReportLog().getLogPath()+"'");
	initArg(request,out);

	String mrd,param, opcode;
	String[] allLogPath = UtilFile.getFileList(RDUtil.getRDSERVER_LOG_DIR(), "report_");
   ...
   if(curIndex >= allLogPath.length)
	...
	if(RdMs.isLogModeFileType()){
		if(L.isUnicodeVersion())
			mrd = cabpath+"/m2soft/uclog_u.mrd";
		else
			mrd = cabpath+"/m2soft/uclog.mrd";
		param = "/rfn ["+cabpath+"/download.jsp?filename="+allLogPath[curIndex]+"]";
	}else{
		String today = new java.text.SimpleDateFormat("yyyy/MM/dd").format(new Date());
		String viewerwin = "";
		if(L.isUnicodeVersion()){
			mrd = cabpath+"/m2soft/uclogdbnchart_u.mrd";
			viewerwin = cabpath+"/m2soft/viewerwin_u.mrd";
		}else{
			mrd = cabpath+"/m2soft/uclogdbnchart.mrd";
			viewerwin = cabpath+"/m2soft/viewerwin.mrd";
		}
		param = "/rfn ["+cabpath+"/rdagent.jsp] "+"/rsn ["+logServiceName+"] "+"/rp "+today+" "+viewerwin;
	}
...
<SCRIPT>
   ...
   function prev(){
      <%if(curIndex > 0){%>
            document.navigation.action = "JLogview.jsp?opcode=<%=curIndex - 1%>";
...
<FORM NAME="navigation" METHOD=POST ACTION="JLogview.jsp">
<%
