...
<%@ include file="/WEB-INF/jsp/egovframework/adm/com/admCommonHidden.jsp"%>
	...
	<div class="listTop">			
		...
		<div class="btnR"><a href="#" class="btn01" onclick="replyPage()"><span>답변등록</span></a></div>
	</div>
    ...
    <div class="tbDetail">
        <table summary="" cellspacing="0" width="100%">
            ...
            <tbody>
                <tr>
                    <th>작성자</th>
                    <td><c:if test="${data.gadmin ne 'ZZ'}">[${data.gadmin}] </c:if>${data.name}(${data.userid})</td>
                </tr>
                <tr class="title">
                    <th>제목</th>
                ...
                <tr class="title">
                    ...
                    <td>
                    	...
                    	<c:if test="${not empty fileList}">
		                    ...
		                    <a href="#none" onclick="fn_download('${file.realfile}', '${file.savefile}', 'studyadminboard')"><c:out value="${file.realfile}"/></a>
		                    <input type="hidden" name="p_savefile"  value="<c:out value="${file.savefile}"/>">
		                    <br/>
                    		</c:forEach>
                    		<input type="hidden" name="p_upload_dir" value="studyadminboard">
                    	</c:if>
                    </td>
                </tr>
            </tbody>
...
<script type="text/javascript">
	...
	function updatePage(){
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.action = "/adm/cmg/sab/studyAdminBoardUpdatePage.do";
		frm.submit();
	}
	
	function replyPage(){
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.action = "/adm/cmg/sab/studyAdminBoardReplyInsertPage.do";
		frm.submit();
	}
	...
	function deleteData(){
		...
		frm.action = "/adm/cmg/sab/studyAdminBoardDelete.do";
		frm.submit();
	}
</script>