	...
	<div class="tbList">
		<table summary="" cellspacing="0" width="100%">
			...
			<c:forEach items="${list}" var="result" varStatus="i">
				<tr>
					<td><c:out value="${(pageTotCnt+1)-result.rn}"/></td>
					<td class="left">
					...
					<td>
						...
						<a href="#none" onclick="whenPersonGeneralPopup('${result.userid}')">${result.name}(${result.userid})</a>
					</td>
					<td>${result.cnt}</td>
				</tr>
			</c:forEach>
...
<script type="text/javascript">
	...
	function parentListPage(){
		...
		frm.action = "/adm/cmg/qna/courseAdminList.do";
	...
	function doView(seq, upfilecnt, userid) {
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.action = "/adm/cmg/qna/courseAdminQnaView.do";
		frm.p_seq.value = seq;
	...
	function insertPage(){
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.action = "/adm/cmg/qna/courseAdminQnaInsertPage.do";
		frm.target = "_self";
		frm.submit();
	}
