        ...
        <table summary="" cellspacing="0" width="100%">
            ...
            <tbody>
                <tr class="title">
                    <th>제목</th>
                    <td>
                    	<input type="text" name="p_title" id="p_title" value="<c:out value="${data.title}"/>"  size="80">
                ...
                <tr class="title">
                    <th>공지구분</th>
                    <td>
                    	<ui:code id="p_types" selectItem="${data.types}" gubun="" codetype="0008"  upper="" title="" className="" type="select" selectTitle="" event="" />
                    </td>
                </tr>
                <tr class="title">
                    <th>내용</th>
                    <td>
                    	<c:set var="content" value="${data.adcontent}"/>
                ...
                <tr class="title">
                    <th>첨부파일</th>
                    <td>
                    	<c:if test="${not empty data.upfile}">
		                    <a href="#none" onclick="fn_download('${data.realfile}', '${data.upfile}', 'bulletin')"><c:out value="${data.realfile}"/></a>
		                    <input type="checkbox"  name="p_filedel" value="Y"> (삭제시 체크)<br/>
							<input type="hidden" name="p_realfile"  value="<c:out value="${data.realfile}"/>">
							<input type="hidden" name="p_savefile"  value="<c:out value="${data.upfile}"/>">
                    	</c:if>
...
<script type="text/javascript">
	...
	function updateData(){
		...
		frm.action = "/adm/cmg/not/courseNoticeUpdate.do"
		frm.submit();
	}
