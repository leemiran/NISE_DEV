	...
	<div class="tbList">
		<table summary="" cellspacing="0" width="100%">
			...
			<c:forEach items="${list}" var="result" varStatus="i">
				<tr>
					<td>${i.count}</td>
					<td class="left"><a href="#none" onclick="doView('${result.seq}', '${result.upfilecnt}', '${result.userid}')">${result.title}</a></td>
					<td>
...
<script type="text/javascript">
	...
	function parentListPage(){
		...
		frm.action = "/adm/cmg/stu/studyAdminDataList.do";
	...
	function doView(seq, upfilecnt, userid) {
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.action = "/adm/cmg/stu/studyAdminDataBoardView.do";
		frm.p_seq.value = seq;
	...
	function insertPage(){
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.action = "/adm/cmg/stu/studyAdminDataInsertPage.do";
		frm.target = "_self";
		frm.submit();
	}
