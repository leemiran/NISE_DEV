        ...
        <table summary="" cellspacing="0" width="100%">
            ...
            <tbody>
                <tr>
                    <th>작성자</th>
                    <td>
                    <c:if test="${data.gadmin ne 'ZZ'}">[${data.gadmin}] </c:if>
                    <a href="#none" onclick="whenPersonGeneralPopup('${data.userid}')">${data.name}(${data.userid})</a>
                    </td>
                </tr>
                <tr>
                    <th>상담분류</th>
                    <td><c:out value="${data.sangdamGubunName}"/></td>
                </tr>
                <tr>
                    <th>공개여부</th>
                    <td><c:out value="${data.isopen eq 'Y' ? '공개' : '비공개'}"/></td>
                </tr>
                <tr class="title">
                    <th>제목</th>
...
<script type="text/javascript">
	...
	function updatePage(){
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.action = "/adm/cmg/qna/courseAdminQnaUpdatePage.do";
		frm.submit();
	}
	
	function replyPage(){
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.action = "/adm/cmg/qna/courseAdminQnaReplyInsertPage.do";
		frm.submit();
	}
	...
	function deleteData(){
		...
		frm.action = "/adm/cmg/qna/courseAdminQnaDelete.do";
		frm.submit();
	}
</script>