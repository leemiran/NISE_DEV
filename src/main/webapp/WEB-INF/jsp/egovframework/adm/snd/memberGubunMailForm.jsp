...
<form name="<c:out value="${gsPopForm}"/>" id="<c:out value="${gsPopForm}"/>" method="post">
	...
	<input type="hidden" id="returnPage" name="memberGubunMailForm">
			...
			<table summary="제목,발송대상,내용으로 구성" width="97%" class="tbDetail">
				...
				<tbody>
					...
					<tr>
						<th scope="row">템플릿</th>
						<td style="margin-top:0px;margin-bottom:0px;">
							<select id="templeteSeq" name="templeteSeq" onchange="getTemplete();">
								<c:choose>
									<c:when test="${empty templeteList}">
										<option value="">템플릿을 등록해 주세요.</option>
									</c:when>
									<c:otherwise>
										<option value="">선택</option>
										<c:forEach var="templete" items="${templeteList}">
											<option value="${templete.templeteSeq}">${templete.title}</option>
										</c:forEach>
									</c:otherwise>
								</c:choose>
							</select>
					...
					<tr>
						...
						<td style="margin-top:0px;margin-bottom:0px;">
							<input type="radio" name="t_emp_gubun" id="t_emp_gubun" <c:if test="${t_emp_gubun eq 'T' }">checked</c:if> value="T" style="cursor:pointer;" > 교원(<fmt:formatNumber value="${memberGubunCount.tCount}" type="number"/>)
							<input type="radio" name="t_emp_gubun" id="t_emp_gubun" <c:if test="${t_emp_gubun eq 'E' }">checked</c:if> value="E" style="cursor:pointer;"> 보조인력(<fmt:formatNumber value="${memberGubunCount.eCount}" type="number"/>)
							<input type="radio" name="t_emp_gubun" id="t_emp_gubun" <c:if test="${t_emp_gubun eq 'R' }">checked</c:if> value="R" style="cursor:pointer;"> 교육 전문직(<fmt:formatNumber value="${memberGubunCount.rCount}" type="number"/>)
							<input type="radio" name="t_emp_gubun" id="t_emp_gubun" <c:if test="${t_emp_gubun eq 'P' }">checked</c:if>value="P" style="cursor:pointer;"> 일반회원(학부모 등)(<fmt:formatNumber value="${memberGubunCount.pCount}" type="number"/>)
						</td>
					</tr>
					
					<tr>
						<th rowspan="2">회원검색</th>
						<td style="margin-top:0px;margin-bottom:0px;">																					
							...
							<a href="#none" class="btn_search" onclick="sendMemberSerarch()"><span>검색</span></a> 
							--%>
							...
							<table summary="" cellspacing="0" width="100%">
								...
								<c:forEach items="${list}" var="result" varStatus="i">
									<tr>
										...
										<td><input type="checkbox" id="_Array_p_key1" name="_Array_p_key1" value="<c:out value="${result.email}"/>,<c:out value="${result.name}"/>,<c:out value="${result.userid}"/>,<c:out value="${result.rdate}"/>"></td>
									</tr>
								</c:forEach>
								</tbody>
							</table>															
...
<script type="text/javascript">
	...
	function sendMail(){
	    ...
	    var cnt=0;
	    for(i=0; i<thisForm.t_emp_gubun.length; i++){
	    	if(thisForm.t_emp_gubun[i].checked){
	    		cnt++;	
	    	}
	    }
	    if(cnt==0){
	    ...
	    var target = thisForm._Array_p_key1;
		var _Array_p_key1Cnt = 0;
		if( target ){
			if(target.length){
				for( i=0; i<target.length; i++ ){
					if( target[i].checked ) _Array_p_key1Cnt++;
				}
			}else{
				if( target.checked ) _Array_p_key1Cnt++;
			}
		}
		if( _Array_p_key1Cnt == 0 && cnt==0){
			alert("대상을 선택하세요.");
			return;
		}
		
	    //alert(thisForm.t_emp_gubun[0].value);
 ...
 */
	...
	function sendMemberSerarch() {
		...
		var url = '/adm/snd/memberGubunMailForm.do';
			
		//window.open("","sendMemberSerarchPopup","width=1000,height=800,scrollbars=yes");
		//thisForm.target = "sendMemberSerarchPopup";
		thisForm.action = url;
		thisForm.submit();
	}
	...
	******************************************************** */
	function doPageList(pageNo) {
		...
		thisForm.action = "/adm/snd/memberGubunMailForm.do";
	...
	function getTemplete() {
		if('' == $('#templeteSeq option:selected').val()) {
			oEditors.getById["textAreaContent"].exec("SET_CONTENTS", [initContent]);
		} else {
			$.ajax({  
				url: "<c:out value="${gsDomainContext}" />/adm/snd/getTempleteContent.do",		
				method: 'post',
				data: {templeteSeq: $('#templeteSeq option:selected').val()
				},
				dataType: 'json',
				contentType : "application/json:charset=utf-8",			 
				success: function(data) {
					oEditors.getById["textAreaContent"].exec("SET_CONTENTS", [data.content]);
				},    
				error: function(xhr, status, error) {   
					alert(status);   
					alert(error);    
	...
	$(function() {
	    initContent = $('#textAreaContent').val();
	});
</script>