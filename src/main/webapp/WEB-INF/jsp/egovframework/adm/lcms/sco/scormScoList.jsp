...
<body style="background: none;">
	...
	<div class="listTop">	
		<div class="btnR"><a href="#" class="btn01" onclick="parent.doMoveOrg('DOWN')"><span>▼</span></a></div>
		<div class="btnR"><a href="#" class="btn01" onclick="parent.doMoveOrg('UP')"><span>▲</span></a></div>
		...
		<div class="btnR"><a href="#" class="btn01" onclick="parent.doMetadata('ORG')"><span>메타테이터관리</span></a></div>
	...
	<div class="tbDetail" style="padding-left:0px;">
		<table summary="" cellspacing="0" width="100%">
			...
			<tbody>
				<tr class="title">
					<th>등록자</th>
					<td class="borderR"><c:out value="${info.name}"/></td>
					<th>등록일</th>
					<td><c:out value="${info.indate}"/></td>                       
				</tr>
			</tbody>
		</table>
	</div>
		<input type="checkbox" name="allCheck" id="allCheck" onclick="allChecked()">전체선택
	<c:set var="oldSeq" value=""></c:set>
		<table summary="차시리스트" id="orgTable" width="100%">
	...
	<c:forEach items="${list}" var="result">
		<c:if test="${result.orgSeq != oldSeq && oldSeq != ''}">
						</div>
					</td>
				</tr>
		</c:if>
		<c:if test="${result.orgSeq != oldSeq}">
				<tr>
					<td class="subject">
			<c:set var="oldSeq" value="${result.orgSeq}"/>
						<br/>
						<p style="font-size:12px; text-align:left;">
							<input name="chk" id="chk" type="checkbox" value="${result.orgSeq}" class="orgCheck"/>
							<a href="#none" onclick="doShowList('<c:out value="${result.orgSeq}"/>')"><c:out value="${result.orgTitle}"/></a>
						</p>
						<div class="edu" id="${result.orgSeq}" style="display:none">    
		</c:if>
							<div>                     
							<dl>
		<c:choose>
			<c:when test="${result.highItemSeq == 0}">
								<dt>
									<c:out value="${result.itemTitle}"/>
									<c:if test="${not empty result.itemIdRef}">
										<a href="#none" onclick="parent.doContentFile('<c:out value="${result.itemSeq}"/>')"><img src="/images/lcms/icon_mm.gif" alt="형상관리" title="형상관리"/></a>
										<a href="#none" onclick="parent.doMetadataItem('ITM', '<c:out value="${result.itemSeq}"/>')"><img src="/images/lcms/icon_data.gif" alt="메타데이터관리" title="메타데이터관리"/></a>
									</c:if>
								</dt>
			</c:when>
			<c:otherwise>
