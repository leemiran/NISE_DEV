...
<body style="background: none;">
	...
	<div class="listTop">	
		...
		<div class="btnR"><a href="#" class="btn01" onclick="parent.doModuleUpdatePopup()"><span>수정</span></a></div>
	</div>
	...
	<c:if test="${result.module != oldModule}">
		...
		<c:set var="oldModule" value="${result.module}"/>
						...
						<div class="edu" id="${result.module}" style="display:none">                         
							...
							<div>  
							<dl>
							<c:if test="${result.depth != 3}">
								<dt>
									<a href="#none" onclick="parent.doUpdatePopup('<c:out value="${result.module}"/>', '<c:out value="${result.lesson}"/>')"><c:out value="${result.lessonName}"/></a>
									<c:if test="${contentType == '06' && result.pageCount > 0}">(<c:out value="${result.pageCount}"/>p)</c:if>
									<c:if test="${result.progressYn == 'Y'}">
									<a href="#none" onclick="parent.doContentFile('<c:out value="${result.module}"/>','<c:out value="${result.lesson}"/>')"><img src="/images/lcms/icon_mm.gif" alt="형상관리" title="형상관리"/></a>
