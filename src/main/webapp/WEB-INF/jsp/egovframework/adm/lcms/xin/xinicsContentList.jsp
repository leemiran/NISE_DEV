...
<form name="<c:out value="${gsMainForm}"/>" id="<c:out value="${gsMainForm}"/>" method="post" action="">
	...
	<input type="hidden" name="contentType" id="contentType" value="X"/>
	<input type="hidden" name="preview"     id="preview"    value="Y"/>
	<input type="hidden" name="pageIndex"   id="pageIndex"  value="${pageIndex}"/> 
	<input type="hidden" name="returnUrl"     id="returnUrl"    value="Y"/>
		...
		<table summary="" cellspacing="0" width="100%">
			...
			<c:forEach items="${list}" var="result">
				<tr>
					...
					<td class="subject">
						<a href="#none" onclick="doOrgView('<c:out value="${result.subj}"/>', <c:out value="${result.orgCount}"/>)">
...
var oldTr = "";
	 ...
	 ******************************************************** */
	 function doPageList() {
		 ...
		 frm.action = "/adm/lcms/xin/xinicsContentList.do";
		 frm.target = "_self";
		 frm.submit();
	}
	...
	// 차시리스트 오픈
	function doOrgView( p_subj, orgCnt ){
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		if( orgCnt == 0 ){
			alert("등록된 차시정보가 없습니다.");
			return;
		}
		this.doRemoveTr();
		if( oldTr != p_subj ){
			...
			frm.action = "/adm/lcms/xin/xinicsOrgList.do";
	...
	function doContentFile( p_module, p_lesson ){
		var frm = eval('document.<c:out value="${gsMainForm}"/>');
		frm.module.value = p_module;
		frm.lesson.value = p_lesson;
		frm.returnUrl.value = "/adm/lcms/xin/xinicsContentList.do";
		frm.target = "_self";
	...
	function doPreview(p_module, p_lesson) {  
		...
		if( selectRow >= 0 ){
			...
			lesson = eval("iframe"+p_subj).document.getElementById("lesson_"+orgSeq).value;
		}else{
			return;
		}
		...
		window.open('', 'preview', 'top=100px, left=100px, height=660px, width=1024px');
		frm.target = "preview";
