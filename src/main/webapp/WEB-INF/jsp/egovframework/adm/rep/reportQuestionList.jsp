		...
		<div class="tbList">
			...
			<c:forEach items="${list}" var="result" varStatus="status">
				<tr>
					...
					<td class="left">
						<c:if test="${result.quesCnt > 0 }">
							<a href="#" onclick="questionDetailList('${result.examSubj}');"><c:out value="${result.examSubjnm}"/></a>
						</c:if>
						<c:if test="${result.quesCnt == 0 }">
							<c:out value="${result.examSubjnm}"/>
						</c:if>
					</td>
					<td><c:out value="${result.useYCnt}"/></td>
					<td><c:out value="${result.useNCnt}"/></td>
					<td><c:out value="${result.quesCnt}"/></td>														
...
function doPageList(pageNo) {
	...
	thisForm.action = "/adm/rep/reportQuestionList.do";
...
function insertExam() {	
    ...
    farwindow = window.open("", "reportQuestionViewPopWindow", "toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no, width = 950, height = 400, top=0, left=0");
    thisForm.target = "reportQuestionViewPopWindow";
    //thisForm.target = "_self";
    thisForm.action = "/adm/rep/reportQuestionView.do";
	thisForm.submit();
...
function questionDetailList(subj){
	...
	farwindow = window.open("", "reportQuestionSubjPopWindow", "toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no, width = 900, height = 700, top=0, left=0");
	thisForm.p_exam_subj.value = subj;	
	thisForm.action = "/adm/rep/reportQuestionSubjList.do";
    thisForm.target = "reportQuestionSubjPopWindow";
	thisForm.submit();
