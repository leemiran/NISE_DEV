...
<!--    public final static String MULTI_QUESTION   = "4";      // 다답식-->
				...
				<c:forEach items="${PaperQuestionExampleList}" var="result">
					<c:if test="${result.examnum ne temp_examnum}">
						...
						<div class="conbox">
							<p class="que1"><B>(${result.lessonnum }차시) 문제 ${idx}.</B></p>
							<p class="que2">
					...
					<c:choose>
						<c:when test="${result.examtype eq '1'}">
									<li><input type="radio" class="chr" name="${result.examnum}" value="${result.selnum}" <c:if test="${result.isanswer eq 'Y' }">checked</c:if> style="cursor:pointer" /> ${result.selnum}) ${result.seltext}</li>
						</c:when>
