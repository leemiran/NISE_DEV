...
<div id="mystudy"><!-- 팝업 사이즈 800*650 -->
	<div class="con2">
		<div class="popCon">
			<div class="myheader">
				<p class="mytit">시험문제 수정</p>
			</div>
			
			<!-- button -->
			<ul class="btnCen">
				<li><a href="javascript:PaperExamAction('update')" class="pop_btn01"><span>자동대체</span></a></li>
				<li><a href="javascript:PaperExamChange('change')" class="pop_btn01"><span>선택대체</span></a></li>
			</ul>
			<!-- // button -->
...
<!--    public final static String MULTI_QUESTION   = "4";      // 다답식-->
				...
				<c:forEach items="${PaperQuestionExampleList}" var="result">
					<c:if test="${result.examnum ne temp_examnum}">
						...
						<div class="conbox">
							...
							<p class="que1"> 
								<input type="checkbox" name="p_checks" id="p_${result.examnum}" class="p_subj" value="${result.examnum}">
				...
				<div id="noprint">
					<ul class="btnR MT10">
						<!-- <li><a href="#none" class="btn02" onclick="onPrint()" id="bottomImg1" ><span>인쇄하기</span></a></li> -->						
...
function PaperExamAction(p_process) {
	...
	thisForm.action = '/adm/exm/examBankPaperExamAction.do';
...
//평가 문제지 미리보기 
function PaperExamChange(p_process) {
	...
	farwindow = window.open("", "PaperExamChangePopUpWindow", "toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no, width = 1000, height = 667, top=0, left=0");    
	
	thisForm.target = "PaperExamChangePopUpWindow";
	thisForm.action = "/adm/exm/examBankPaperExamChange.do";
	thisForm.submit();
