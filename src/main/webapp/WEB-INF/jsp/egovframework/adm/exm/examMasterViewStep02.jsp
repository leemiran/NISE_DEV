...
<div id="popwrapper">
	<div class="popIn">
		...
		<div>
			<p class="pop_tit">평가마스터관리 Step 2</p>			
		</div>
...
<c:if test="${not empty view}">                        
		...
		<ul class="btnCen">
			...
			<c:if test="${paperCnt == 0}">
				<li><a href="javascript:masterAction('update')" class="pop_btn01"><span>저 장</span></a></li>
			</c:if>
			
			
			</c:if>
			<c:if test="${empty view}">
				<li><a href="javascript:masterAction('insert')" class="pop_btn01"><span>저 장</span></a></li> 
			</c:if>
...
function masterAction(p_process) {
	  if (!chkData()) {
	    return;
	  }
	  ...
	  thisForm.action = '/adm/exm/examMasterAction.do';
