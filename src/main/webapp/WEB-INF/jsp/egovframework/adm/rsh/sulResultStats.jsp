...
<%@ include file="/WEB-INF/jsp/egovframework/adm/com/admCommonHidden.jsp"%>
	...
	<!-- 검색박스 시작-->
	<c:import url="/com/sch/admSearchBars.do">
		<c:param name="selectViewType"      value="CC_year"							>조회조건타입 : 타입별 세부조회조건		</c:param>
		<c:param name="selectParameter"     value=""							>조회조건 추가 : admSearchParameter.jsp 추가	</c:param>
	</c:import>
	<!-- 검색박스 끝 -->
		...
		<div class="sub_tit">
			<h4 id="sulScoreTotalH4">연수 만족도</h4>	
		</div>
		...
		<div class="tbList">
		  <table summary="" cellspacing="0" width="100%">
           <caption>목록</caption>
	        ...
	        <tbody>	
		      <tr>
		        <td class="num"><fmt:formatNumber value="${sulmunStddev1.avgPoint }" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${sulmunStddev1.pointStddev }" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${y1[0]}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${y1[1]}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${y1[2]}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${y1[3]}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${sulmunStddev1.minPoint}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${sulmunStddev1.maxPoint}" type="number" pattern=",##0.00"/></td>
	          </tr>
	        </tbody>
	     </table> 
	    ...
	    <br/>
	    <div class="sub_tit">
			<h4 id="sulScoreTotalH4">교수학습 만족도</h4>	
		</div>
		...
		<div class="tbList">
		  <table summary="" cellspacing="0" width="100%">
           <caption>목록</caption>
	        ...
	        <tbody>	
		      <tr>
		        <td class="num"><fmt:formatNumber value="${sulmunStddev2.avgPoint }" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${sulmunStddev2.pointStddev }" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${y2[0]}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${y2[1]}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${y2[2]}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${y2[3]}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${sulmunStddev2.minPoint}" type="number" pattern=",##0.00"/></td>
		        <td class="num"><fmt:formatNumber value="${sulmunStddev2.maxPoint}" type="number" pattern=",##0.00"/></td>
	          </tr>
	        </tbody>
	     </table>   
	      <br/>  
...
function whenXlsDownLoad() {
	...
	thisForm.action = "/adm/rsh/sulResultStats.do?p_process=xlsdown";
	thisForm.target = "_self";
	thisForm.submit();
}

function fn_sulResultStats_checkEnter(){
	 if(event.keyCode == 13){		 
		 doPageList();
	 }
}
...
function whenXlsDetailDownLoad() {
	...
	thisForm.action = "/adm/rsh/sulResultStats.do?p_process=xlsdetaildown";
	thisForm.target = "_self";
	thisForm.submit();
}
...
function doPageList(pageNo) {
	...
	thisForm.action = "/adm/rsh/sulResultStats.do";
...
function dataCheck(){
	...
	if(thisForm.p_temp_gubun_1.checked==true ){
		gubun_cnt++;
	}
	if(thisForm.p_temp_gubun_2.checked==true ){
		gubun_cnt++;
	}
	if(thisForm.p_temp_gubun_3.checked==true ){
		gubun_cnt++;
	}
	if(thisForm.p_temp_gubun_4.checked==true ){
		gubun_cnt++;
	}
	...
	if(!number_chk_noalert(thisForm.p_per.value)){
		alert("임의 추출값을 숫자로 입력 하세요.");
		thisForm.p_per.focus();
		return false;
	}
	
	if((thisForm.p_per.value < 1) || (thisForm.p_per.value > 100)){
		alert("임의 추출값을 확인 하세요.");
		thisForm.p_per.focus();
		return false;
	}
}
