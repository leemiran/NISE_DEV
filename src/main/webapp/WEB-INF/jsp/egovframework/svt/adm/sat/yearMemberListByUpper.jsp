    ...
    <div class="tbDetail">    
		...
		<thead>
			<tr>
				...
				<th scope="row" style="text-align: center;">공무원</th>
		...
		<tbody>
			<c:forEach items="${resultList}" var="result" varStatus="status">
				<tr>
					<td style="text-align: center;"><fmt:formatNumber value="${result.t}" groupingUsed="true"/></td>
					<td style="text-align: center;"><fmt:formatNumber value="${result.e}" groupingUsed="true"/></td>
					<td style="text-align: center;"><fmt:formatNumber value="${result.r}" groupingUsed="true"/></td>
					<td style="text-align: center;"><fmt:formatNumber value="${result.p}" groupingUsed="true"/></td>
					<td style="text-align: center;"><fmt:formatNumber value="${result.o}" groupingUsed="true"/></td>
...
function chart() {
	...
	pData = google.visualization.arrayToDataTable([
		['Element', '회원', { role: 'style' }, { role: 'annotation' } ],
		['교원', Number(map.get("t"))	, '#555500', Number(map.get("t")) ],
		['보조', Number(map.get("e"))	, '#005500', Number(map.get("e")) ],
		['교육전문직', Number(map.get("r")), '#000055', Number(map.get("r")) ],
		['일반', Number(map.get("p"))	, '#aa0000', Number(map.get("p")) ],
		['공무원', Number(map.get("o")), '#00aa00', Number(map.get("o")) ],
		['기타', Number(map.get("etc")), '#0000aa', Number(map.get("etc")) ]
	]);
}
