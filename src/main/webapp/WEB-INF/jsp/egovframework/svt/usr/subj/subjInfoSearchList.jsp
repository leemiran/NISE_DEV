               ...
               <%--  <div class="courseWrapSubj" style="height:100px;">
                    ...
                    <ul>
                         ...
                         <c:set var="nowMonth">${fn2:getFormatDateNow('MM')}</c:set>
                            ...
                            <select name="search_year" title="연수기간 연도선택">
                         ...
                         <li>
                         과정명 
                         	<input name="search_text" type="text" size="48" value="<c:out value="${search_text}"  escapeXml="true" />" onkeydown="fn_keyEvent('doSearchList')" title="검색어"/>
                ...
                <!-- list table-->
                    <div class="studyList">
                        <table summary="패키지, 과정명, 연수기간, 수강신청으로 구성" cellspacing="0" width="100%">
...
<c:forEach items="${list}" var="result" varStatus="status">
	<tr>
		...
		<td class="num">${fn2:getFormatDate(result.propstart, 'yyyy.MM.dd')}<br>~ ${fn2:getFormatDate(result.propend, 'yyyy.MM.dd')}
		</td>
		<td class="num">${fn2:getFormatDate(result.edustart, 'yyyy.MM.dd')}<br>~ ${fn2:getFormatDate(result.eduend, 'yyyy.MM.dd')}
		</td>
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<c:choose>
							...
							<c:when test="${(('PRF' eq result.upperclass and 'P' eq result.empGubun) or 
								 ('PRF' eq result.upperclass and 'O' eq result.empGubun))
								 or 
								(('EXT' eq result.upperclass and 'T' eq result.empGubun) or 
								 ('EXT' eq result.upperclass and 'R' eq result.empGubun))}">
											...
											<c:choose>
												<c:when test="${result.isPropose2 < result.studentlimit}">
													...
													<a href="#none" onclick="whenSubjPropose('${result.subj}','${result.year}','${result.subjseq}');">
														<img src="/images/user/subj_study_add.gif" alt="신청하기" title="새 창으로 열기"/>
													</a>
												</c:when>
												<c:otherwise>
								...
								<c:if test="${result.propstatus eq 'C'}">									                                   	
                                   	<c:if test="${result.isPropose > 0}">
                                   		<c:if test="${result.chkfinal eq 'Y'}">
                                   			<img src="/images/user/subj_status08.gif" alt="학습중"/>
                                   		</c:if>
                                   		<c:if test="${result.chkfinal ne 'Y'}">
                                   			<img src="/images/user/subj_status07.gif" alt="학습대기"/>
                                   		</c:if>	
                                   	</c:if>
                                   	<c:if test="${result.isPropose == 0}">
                                   		<img src="/images/user/subj_status03.gif" alt="신청마감"/>
...
<c:if test="${empty list}">
	<tr>
		<td colspan="9" class="last">검색된 정보가 없습니다.</td>
	</tr>
</c:if>
