...
<c:choose>
	...
	<c:when test="${resultMap.result eq 'Y' }">
		<script>
		window.onload = function () {
		    window.opener.name = "parentPage"; // 부모창의 이름 설정
		    document.resSciFrm.target = "parentPage"; // 타켓을 부모창으로 설정
		    document.resSciFrm.action = "${pageContext.request.contextPath}${retUrl}";
		    document.resSciFrm.submit();
		    self.close();
		...
		<form name="resSciFrm" method="post">
			<c:choose>
				<c:when test="${retType eq 'join' }">
					...
					<input type="hidden" name="certgubun" value="S"/> <!-- SCI 본인인증: S -->
