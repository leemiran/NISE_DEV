...
<%@ include file="/WEB-INF/jsp/egovframework/usr/com/usrCommonHidden.jsp"%>
	...
	<input type = "hidden" name="p_year" />
	<input type = "hidden" name="p_subjseq" />
	<input type = "hidden" name="p_lec_sel_no" />
	<input type = "hidden" name="p_redirect_url" value="${requestScope["javax.servlet.forward.request_uri"]}" />
		...
		<div class="sub_title">
			<ul>
                <li class="tred">연수지명번호를 입력 후 [저장]을 꼭 클릭해 주세요(예 : 경기 - 학교 - 2014 - 지명번호)</li>
                <li class="tred">학부모과정과 보조인력과정은 입력하지 않습니다.</li>
            </ul>
		</div>
...
$( document ).ready(function() {
	var lecselno_${status.count} = '${result.lecselno}';

	if(lecselno_${status.count})
	{
			if(lecselno_${status.count}.indexOf("-") > -1)
			{
					var arr_lecselno = lecselno_${status.count}.split("-");

					for(var i=0; i<arr_lecselno.length; i++)
					{
						$("#p_lec_sel_no_${status.count}_" + i).val(arr_lecselno[i]);
					}

			}
			else
			{
				$("#p_lec_sel_no_${status.count}_0").val(lecselno_${status.count});
			}
	}
});
//-->
                        ...
                        title="연수지명번호 첫번째 입력">-<input type="text" name="p_lec_sel_no_${status.count}_1" 	id="p_lec_sel_no_${status.count}_1" value="" size="10" maxlength="20"
                        title="연수지명번호 두번째 입력">-<input type="text" name="p_lec_sel_no_${status.count}_2" 	id="p_lec_sel_no_${status.count}_2" value="" size="4" maxlength="4"
                        title="연수지명번호 세번째 입력">-<input type="text" name="p_lec_sel_no_${status.count}_3"	 id="p_lec_sel_no_${status.count}_3" value="" size="4" maxlength="4"
                        title="연수지명번호 네번째 입력">
                        
                        </td>
                        <td class="last num">                        
                        <a href="javascript:;" onclick="whenSave('${result.subj}','${result.year}','${result.subjseq}', 'p_lec_sel_no_${status.count}' )">
...
//상태저장
function whenSave(p_subj, p_year, p_subjseq, p_lec_sel_no){
	...
	thisForm.p_lec_sel_no.value = $("#" +p_lec_sel_no + "_0").val() + "-" + $("#" +p_lec_sel_no + "_1").val() + "-" + $("#" + p_lec_sel_no + "_2").val() + "-" + $("#" + p_lec_sel_no + "_3").val();
