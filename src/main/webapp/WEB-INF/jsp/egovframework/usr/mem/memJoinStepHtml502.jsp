...
<!-- ipin 인증 -->
<%@ include file = "/WEB-INF/jsp/egovframework/svt/cert/ipin/ipinInclude.jsp" %>
<!-- epki 인증 -->
<%-- <%@ include file = "/WEB-INF/jsp/egovframework/svt/cert/epki/epkiInclude.jsp" %> --%>
<!-- chkPlus 인증 -->
<%@ include file = "/WEB-INF/jsp/egovframework/svt/cert/checkplus/cpInclude.jsp" %>
<!-- 부트스트랩  CSS -->
...
<form name="<c:out value="${gsMainForm}"/>" id="<c:out value="${gsMainForm}"/>" method="post" onsubmit="return false;" action="">
	...
	<input type="hidden" name="p_process"/>
	<input type="hidden" name="p_resno" id="p_resno"/>
			...
			<ul class="btnC m15">
			            <li><a href="javascript:reqIpin('join');" title="새창"><img src="/images/user/btn_sil.gif" alt="안심본인인증" /></a></li>
			</ul>     
			...
			<p class="personal_box">
				...
				NICE평가정보(주) : <a href="http://www.vno.co.kr" target="_blank" title="새창"><span class="personal_blue1">http://www.vno.co.kr</span></a> 
				("개인정보 보험법 시행령 제20조"에 의거) 공인인증서, 신용카드, 휴대폰등으로 본인임을 확인하여 별도의 비용이 없습니다.
			</p>
			
			<ul class="btnC m15">
			            <li><a href="javascript:reqIPin('join');" title="새창"/><img src="/images/user/btn_ipins.gif" alt="아이핀인증" /></a></li>
			</ul> 
			 <div class="sub_text_2 m30">
			     <h4>EPKI 인증</h4>                
			 </div>
			...
			<ul class="btnC m15">
			            <li>*주민번호 입력 후 EPKI 인증 버튼을 눌러주세요.</li>
			            <li><label for="p_resno1">주민번호</label> : 
			            ...
			            <a href="#none" name="epkiBtn" id="epkiBtn" title="새창"><img src="/images/user/btn_epki.gif" alt="EPKI인증" /></a></li>
...
<div id="epkidiv">
<form id="epki_form" name="epki_form" action="http://iedu.nise.go.kr/usr/mem/memJoinStep03.do" method="post">
<input type="hidden" name="certgubun" id="certgubun" value="E"/> <!-- EPKI 인증: E -->
<input type="hidden" name="p_resno_userid" id="p_resno_userid" /> <!-- EPKI 인증: E -->
	<div style="display: none">
		<textarea name="reqSecLoginData" >test </textarea>
	</div>
</form>
</div>
...
$(document).ready(function() {
	...
	<c:if test="${!isDevelopmentMode}">
	    ...
	    html5Epki = function() {	
	    	...
	    	if(thisForm.p_resno1.value.search(/\S/) < 0 ){
	            alert( "주민등록번호를 입력하세요.");
	            thisForm.p_resno1.focus();
	            return;
	        }
	    	
	    	if(thisForm.p_resno2.value.search(/\S/) < 0 ){
	            alert( "주민등록번호를 입력하세요.");
	            thisForm.p_resno2.focus();
	            return;
	        }
...
function reqChkPlus() {
	window.open('/usr/mem/checkPlusSafeMain.do', 'popupChk', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
}
</script>
