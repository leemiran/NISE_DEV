	...
	<input type = "hidden" name="p_subjseq" />
		...
		<!-- <p class="floatR"><a href="#none" onclick="open_permission()"><img src="/images/user/btn_dom_print.gif" alt="원격연수원 인가서 인쇄" /></a></p> -->
			<ul>
                ...
                <li>무통장입금, 계좌이체로 결재하신 경우 연수영수증이 발급됩니다.</li>
...
<!--						</a>-->
                        ...
                        <td class=" num">
                        	<a href="#none" onclick="confirmPrint('${result.subj}','${result.year}','${result.subjseq}')">
...
<!--						학부모가 아닌경우 영수증 출력-->
						<c:if test="${result.biyong > '0'}">
							<c:if test="${result.type ne 'OB' && result.type ne 'RE'}">
...
<%--						  	<c:when test="${result.type eq 'SC0030'}">                   --%>
								<!--계좌이체 -->
<%--						  		<a href="javascript:showCashReceipts('believe2024','${result.orderId}','','BANK','service')" class="pop_btn01"><span>계좌이체</span></a>                   --%>
<%--						  	</c:when>                   --%>
